...
| Profiles | active profile `05:84`, profile-addressed setting banks for DPI/buttons/brightness, metadata chunks via `05:88`, delete/unassign via `05:03`, passive profile-cycle HID hint | active target `03 82`, inventory `03 80`, metadata `03 84`, delete `03 06`, profile-addressed DPI/buttons/lighting, passive profile-cycle HID hint | `OpenSnekProbe` research paths on both transports | PARTIAL | USB and BLE now both expose active-profile reads, stored profile banks, UUID/name reads, cycle-ring delete/unassign, and physical profile-button hints. USB storage IDs `2..5` line up with BLE targets `2..5` for DPI/brightness on the attached V3 Pro, but readable banks are not necessarily cycleable profiles because USB still lacks a reliable inventory/list equivalent to BLE `03 80`. Still missing USB metadata writes and USB inventory/list. |
...
- observed V3 Pro USB metadata-write negative probe on June 16, 2026: a guarded `0x05:0x08` offset-`0` UUID/name write attempt against profile `0x05` did not change `0x05:0x88` readback, and profile `0x05` remained intact
