...
| Lighting/effects | class `0x0F` (OpenRazer documented), including V3 Pro USB Custom Frame `0F:03` for volatile 12-cell software effects | scalar brightness (`10 85`/`10 05`), legacy frame stream (`10 84`/`10 04`), and V3 Pro zone static state (`10 83`/`10 03`) | USB zone writes + V3 Pro USB software presets + BLE zone brightness/static-color writes | PARTIAL | OpenSnek ships V3 Pro USB software-driven Custom Frame presets while the app/service is running, and ships static-color Bluetooth lighting for the Basilisk V3 Pro by fanning out per-zone `10 03`/`10 05` writes across `0x01`, `0x04`, and `0x0A`. Advanced BLE effect streaming remains unshipped. |
