...
| Profiles | active profile `05:84`, active selector `05:04`, profile-addressed setting banks for DPI/buttons/brightness, metadata chunks via `05:88`, delete/unassign via `05:03`, passive profile-cycle HID hint | active target `03 82`, inventory `03 80`, metadata `03 84`, delete `03 06`, profile-addressed DPI/buttons/lighting, passive profile-cycle HID hint | `OpenSnekProbe` research paths on both transports | PARTIAL | USB and BLE now both expose active-profile reads, stored profile banks, UUID/name reads, cycle-ring delete/unassign, and physical profile-button hints. USB can select assigned profiles through `05:04`, and rejected selectors can infer non-cycleable readable banks with side effects. Still missing safe USB metadata writes and side-effect-free USB inventory/list. |
