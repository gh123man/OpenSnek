...
| Profiles | profile-addressed setting banks for DPI, buttons, and brightness; metadata chunks via `05:88`; delete/unassign via `05:03`; passive profile-cycle HID hint; summary hint `00:87` | active target `03 82`, inventory `03 80`, metadata `03 84`, delete `03 06`, profile-addressed DPI/buttons/lighting, passive profile-cycle HID hint | `OpenSnekProbe` research paths on both transports | PARTIAL | USB and BLE now both expose stored profile banks, UUID/name reads, cycle-ring delete/unassign, and physical profile-button hints. USB storage IDs `2..5` line up with BLE targets `2..5` for DPI/brightness on the attached V3 Pro, but readable banks are not necessarily cycleable profiles because `00:87` still reported count `3`. Still missing USB metadata writes, USB inventory/list, and a direct USB active-profile ID equivalent to BLE `03 82`. |
