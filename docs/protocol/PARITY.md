...
| Profiles | active profile `05:84`, active selector `05:04`, profile-addressed setting banks for DPI/buttons/brightness, metadata chunks via `05:88`, delete/unassign via `05:03`, passive profile-cycle HID hint | active target `03 82`, active selector `03 02`, inventory `03 80`, metadata `03 84`, delete `03 06`, profile-addressed DPI/buttons/lighting, passive profile-cycle HID hint | `OpenSnekProbe` research paths on both transports | PARTIAL | USB and BLE now both expose active-profile reads/selectors, stored profile banks, UUID/name reads, cycle-ring delete/unassign, and physical profile-button hints. USB `05:04` and BLE `03 02` select assigned profiles, and rejected selectors can infer non-cycleable readable banks with side effects. Still missing safe USB metadata writes and side-effect-free USB inventory/list. |
...
- observed V3 Pro USB profile-summary candidates on June 16, 2026: `0x05:0x80` returned `03`, `0x05:0x8A` returned `05`, and `0x05:0x81` returned `05 01 03 05`, but profile `5` still rejected active selection, so these remain summary/list hints rather than a trusted side-effect-free inventory
- observed V3 Pro USB metadata-write failure on June 16, 2026: `0x05:0x08` is unsafe as a metadata-only command. A fuller `0x06:0x8E` / `0x05:0x02` / multi-chunk `0x05:0x08` sequence erased profile `0x05` metadata to UUID `ffffffff-ffff-ffff-ffff-ffffffffffff`, name `nil`, and disturbed settings. Settings were restored through validated DPI/lighting writes; metadata remains unrecovered.
- observed V3 Pro USB create/name mapping negatives on June 16, 2026: the Bluetooth guarded create/rewrite path has no safe USB equivalent yet. `0x06:0x8E` returned stable profile-independent data (`00 64 00 04 c0 00 00 04 a8 00 00 00 15`), `0x05:0x82` rejected profile arguments, and `0x05:0x8A` ignored profile arguments, so USB can read names through `0x05:0x88` but must not create/rename UUID/name metadata on-device.
...
- shipped client behavior: the V3 Pro still uses the validated USB button-function protocol for direct slot read/write/readback work, and active-profile hydration can use `0x05:0x84`; full onboard profile CRUD UI remains gated on safe inventory and metadata/create semantics
...
- not yet decoded enough to ship: full profile create allocation, safe standalone metadata rename/update, and stable clutch/profile-button restore-remap behavior
