...
| Profiles | inventory `05:81` / count `05:80`, active profile `05:84`, active selector `05:04`, profile-addressed setting banks for DPI/buttons/brightness, metadata chunks via `05:88/08`, create/assign prelude `05:02`, delete/unassign via `05:03`, passive profile-cycle HID hint | active target `03 82`, active selector `03 02`, inventory `03 80`, metadata `03 84/04`, create/assign through `03 06` + `08 05`/`08 07` + `03 05` + metadata/content writes, delete/unassign `03 06`, profile-addressed DPI/buttons/lighting, passive profile-cycle HID hint | `OpenSnekProbe` research paths on both transports | PARTIAL | USB and BLE now have core CRUD parity for mapped profile surfaces: active-profile reads/selectors, inventory/list, stored profile banks, UUID/name reads/writes on assigned profiles, explicit-target create/assign, cycle-ring delete/unassign, and physical profile-button hints. Both transports reject direct metadata writes to unassigned banks until the create/assign prelude runs. Full product support is still gated on transaction/readback handling and unmapped advanced profile surfaces. |
...
- shipped client behavior: the V3 Pro still uses the validated USB button-function protocol for direct slot read/write/readback work, and active-profile hydration can use `0x05:0x84`; full onboard profile CRUD UI remains gated on productionizing the mapped metadata/create transaction and covering remaining profile surfaces
...
- not yet decoded enough to ship: automatic profile create allocation policy, broad transaction/readback integration for product CRUD, advanced profile surfaces, and stable clutch/profile-button restore-remap behavior
