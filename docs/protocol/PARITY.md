...
| Profiles | inventory `05:81` / count `05:80`, active profile `05:84`, active selector `05:04`, profile-addressed setting banks for DPI/buttons/brightness, metadata chunks via `05:88/08`, create/assign prelude `05:02`, delete/unassign via `05:03`, passive profile-cycle HID hint | active target `03 82`, active selector `03 02`, inventory `03 80`, metadata `03 84/04`, delete `03 06`, profile-addressed DPI/buttons/lighting, passive profile-cycle HID hint | `OpenSnekProbe` research paths on both transports | PARTIAL | USB and BLE now both expose active-profile reads/selectors, inventory/list, stored profile banks, UUID/name reads/writes, cycle-ring delete/unassign, and physical profile-button hints. USB create/assign is mapped with `05:02` + full `05:08`, but it can disturb profile content and must be followed by explicit stored-content writes/readback before shipping user-facing CRUD. |
