...
| Lighting: brightness + static color | `Shipped` | `Shipped` | USB ships three-zone brightness/effect controls and exposes static custom-frame editing for all 12 addressable LEDs (1 logo + 1 scroll wheel + 10 underglow) through `Class 0x0F Cmd 0x03`; see [docs/research/BASILISK_V3_PRO_PERLED_UNDERGLOW.md](./research/BASILISK_V3_PRO_PERLED_UNDERGLOW.md). BT ships per-zone brightness and per-zone static color on `0x01`, `0x04`, and `0x0A`. |
