...
final class V3ProUSBSoftwareLightingUITests: OpenSnekHardwareUITestCase {
    ...
    func testLightingTabsAndAdvancedApplyReplacementStayConnected() throws {
        ...
        XCTAssertTrue(app.descendants(matching: .any)["lighting-card-summary-text"].waitForExistence(timeout: 1), "Collapsed lighting summary did not appear")
        ...
        XCTAssertTrue(app.descendants(matching: .any)["lighting-zone-logo-orb-button"].waitForExistence(timeout: 1), "Individual zone orb controls did not appear")
    ...
    private func expandLightingCardIfNeeded(timeout: TimeInterval = 2) {
        let tabPicker = app.descendants(matching: .any)["lighting-card-tab-picker"]
        if tabPicker.exists { return }

        let lightingCard = app.descendants(matching: .any)["lighting-card"]
        if lightingCard.exists { scrollElementToVisible(lightingCard) }
    ...
    private func assertOnboardLightingControlsExist() {
        XCTAssertTrue(app.descendants(matching: .any)["lighting-brightness-slider"].waitForExistence(timeout: 1), "Onboard brightness slider did not appear")
        XCTAssertTrue(app.descendants(matching: .any)["lighting-effect-picker"].waitForExistence(timeout: 1), "Onboard preset picker did not appear")
        XCTAssertTrue(app.descendants(matching: .any)["lighting-zone-mode-picker"].waitForExistence(timeout: 1), "Onboard zone-mode picker did not appear")
        XCTAssertTrue(app.descendants(matching: .any)["lighting-all-zones-orb-button"].waitForExistence(timeout: 1), "All-zones color orb did not appear")
    }

    private func assertAdvancedLightingControlsExist() {
        let expectedIdentifiers = [
            "software-lighting-apply-on-connect-checkbox", "software-lighting-preset-picker", "software-lighting-speed-slider", "software-lighting-brightness-slider", "software-lighting-palette-list", "software-lighting-palette-reset-button", "software-lighting-palette-add-button",
            "software-lighting-palette-0-orb-button", "software-lighting-apply-button"
        ]
    ...
    private func applySoftwareLightingAndAssertRunning(label: String?) throws {
        ...
        let frameEvent = waitForEvent(named: "usbCommand", timeout: 5) { event in event.timestamp >= clickedAt.timeIntervalSince1970 - 0.1 && event.command?.name == "usbLightingCustomFrame" && expectedScope.matches(event.scope) }
        XCTAssertNotNil(frameEvent, "Software lighting Apply did not produce a Custom Frame USB command")

        if let label {
            XCTAssertTrue(waitForStatusText(containing: "Running \(label)", timeout: 3), "Software lighting status did not update to Running \(label)")
        } else {
            XCTAssertTrue(waitForStatusText(containing: "Running", timeout: 3), "Software lighting status did not report a running effect")
        }
        XCTAssertTrue(app.descendants(matching: .any)["software-lighting-stop-button"].waitForExistence(timeout: 1), "Software lighting Stop button did not appear after Apply")
    ...
    private func selectSegment(_ label: String, in picker: XCUIElement, normalizedX: CGFloat) {
        ...
        if let option = firstExistingElement(in: [picker.descendants(matching: .button)[label], app.buttons[label], app.radioButtons[label], app.descendants(matching: .any)[label]], timeout: 0.5) {
            clickElement(option)
            return
        }
    ...
    private func waitForStatusText(containing expected: String, timeout: TimeInterval) -> Bool {
        ...
        repeat {
            if statusText.exists, elementText(statusText).contains(expected) { return true }
    ...
    private func waitForStatusText(absent text: String, timeout: TimeInterval) -> Bool {
        ...
        let statusText = app.descendants(matching: .any)["software-lighting-status-text"]
        repeat {
            if !statusText.exists || !elementText(statusText).contains(text) { return true }
            RunLoop.current.run(until: Date().addingTimeInterval(0.05))
        } while Date() < deadline
