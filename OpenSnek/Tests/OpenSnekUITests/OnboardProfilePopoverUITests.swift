...
class OnboardProfilePopoverUITests: OpenSnekHardwareUITestCase {
    ...
    override var expectedScope: HardwareDeviceScope { .v3ProUSB }
    ...
    func testProfileManagerCoversCRUDSwitchingAndRestoresActiveProfile() throws {
        ...
        let initialState = try XCTUnwrap(latestExpectedScopedState(), "Expected \(expectedScope.description) to publish hydrated state before profile CRUD coverage")
        let initialActiveProfileID = try XCTUnwrap(initialState.activeOnboardProfile, "Expected \(expectedScope.description) state to include the active onboard profile")
        ...
        for profileID in profileIDs { XCTAssertTrue(profileRow(profileID).exists, "Profile row \(profileID) should be listed") }

        guard let targetProfileID = profileIDs.first(where: { $0 > 1 && isProfileEmpty($0) }) else { throw XCTSkip("No empty non-base onboard profile slot is available for safe CRUD coverage") }
        let suffix = String(UUID().uuidString.prefix(4))
        let createdName = "OS CRUD \(suffix)"
        let renamedName = "OS Ren \(suffix)"

        try selectProfile(targetProfileID)
        XCTAssertTrue(app.staticTexts["Load Profile"].waitForExistence(timeout: 2), "Empty slot details should present local profiles as loadable, not replaceable")
        XCTAssertFalse(app.descendants(matching: .any)["onboard-profile-create-button"].waitForExistence(timeout: 1), "Empty slot details should use the consolidated New Profile flow")
        XCTAssertFalse(app.descendants(matching: .any)["onboard-profile-name-field"].exists, "Empty slot details should not show the onboard name field before a profile is loaded")

        try createFreshLocalProfileAndWaitForAutoAssign(named: createdName)
        createdProfileID = targetProfileID
        XCTAssertTrue(waitForProfileRow(targetProfileID, containing: createdName, timeout: actionTimeout), "Created profile row did not show \(createdName)")
        XCTAssertTrue(waitForActiveProfile(targetProfileID, timeout: actionTimeout), "Created profile \(targetProfileID) did not become active")
        XCTAssertTrue(waitForPill(containing: createdName, timeout: 2), "Profile pill did not reflect the created profile name")

        let renameButton = try requireElement("onboard-profile-rename-button", timeout: 2)
        XCTAssertTrue(waitForElementEnabled(renameButton, timeout: actionTimeout), "Rename should be enabled for the created profile")
        ...
        XCTAssertTrue(waitForProfileRow(targetProfileID, containing: renamedName, timeout: actionTimeout), "Renamed profile row did not show \(renamedName)")
        XCTAssertTrue(waitForPill(containing: renamedName, timeout: 2), "Profile pill did not reflect the renamed profile name")

        try selectProfile(initialActiveProfileID)
        XCTAssertTrue(waitForActiveProfile(initialActiveProfileID, timeout: actionTimeout), "Original profile \(initialActiveProfileID) did not become active when selected")

        try selectProfile(targetProfileID)
        XCTAssertTrue(waitForActiveProfile(targetProfileID, timeout: actionTimeout), "Created profile \(targetProfileID) did not become active when reselected")
        ...
        XCTAssertTrue(waitForProfileRow(targetProfileID, containing: "None", timeout: actionTimeout), "Deleted profile \(targetProfileID) did not return to an empty slot")
        didDeleteCreatedProfile = true
        createdProfileID = nil

        try selectProfile(initialActiveProfileID)
        XCTAssertTrue(waitForActiveProfile(initialActiveProfileID, timeout: actionTimeout), "Failed to restore original active profile \(initialActiveProfileID)")
    ...
    private func assertProfilePillMatchesStatusPillHeight() throws {
        ...
        XCTAssertEqual(profilePill.frame.height, statusPill.frame.height, accuracy: 1, "Profile pill should use the same vertical sizing as the connected status pill")
    }
    ...
    private func openProfileManager(timeout: TimeInterval = 5) throws {
        ...
        XCTAssertTrue(app.descendants(matching: .any)["onboard-profiles-card"].waitForExistence(timeout: timeout), "Onboard profile manager popover did not appear")
        XCTAssertTrue(profileRow(1).waitForExistence(timeout: timeout), "Base onboard profile row did not appear in the popover")
    }

    private func openProfileManagerIfPossible(timeout: TimeInterval = 2) -> Bool {
        if app.descendants(matching: .any)["onboard-profiles-card"].exists { return true }
    ...
    private func visibleProfileIDs() -> [Int] { (1...8).filter { profileRow($0).exists } }

    private func profileRow(_ profileID: Int) -> XCUIElement { app.descendants(matching: .any)["onboard-profile-row-\(profileID)"] }

    private func profileRowActiveBadge(_ profileID: Int) -> XCUIElement { app.descendants(matching: .any)["onboard-profile-row-\(profileID)-active-badge"] }
    ...
    private func selectProfile(_ profileID: Int) throws {
        ...
        XCTAssertTrue(waitForElementEnabled(row, timeout: actionTimeout), "Profile row \(profileID) was not enabled for selection")
        clickElement(row)
        XCTAssertTrue(app.descendants(matching: .any)["local-profile-create-button"].waitForExistence(timeout: 2), "Profile details did not appear after selecting profile \(profileID)")
    }
    ...
    private func isProfileEmpty(_ profileID: Int) -> Bool { profileRowText(profileID).contains("None") }

    private func profileRowText(_ profileID: Int) -> String {
        let row = profileRow(profileID)
        return [elementText(row), (row.value as? String) ?? ""].joined(separator: " ")
    }

    private func waitForProfileRow(_ profileID: Int, containing expectedText: String, timeout: TimeInterval) -> Bool {
    ...
    private func waitForElementEnabled(_ element: XCUIElement, timeout: TimeInterval) -> Bool { waitUntil(timeout: timeout) { element.exists && element.isEnabled } }

    private func waitForElementDisabled(_ element: XCUIElement, timeout: TimeInterval) -> Bool { waitUntil(timeout: timeout) { element.exists && !element.isEnabled } }
    ...
    private func deleteCreatedProfileIfNeeded() {
        guard let createdProfileID, !didDeleteCreatedProfile else { return }
        ...
        guard profileRow(createdProfileID).waitForExistence(timeout: 2), !isProfileEmpty(createdProfileID) else { return }
        ...
        guard deleteButton.waitForExistence(timeout: 2), waitForElementEnabled(deleteButton, timeout: 2) else {
    ...
    private func restoreOriginalActiveProfileIfNeeded() {
        guard let originalActiveProfileID else { return }
        guard openProfileManagerIfPossible() else {
        ...
        if waitForActiveProfile(originalActiveProfileID, timeout: actionTimeout) { self.originalActiveProfileID = nil } else { XCTFail("Could not restore original active profile \(originalActiveProfileID) during cleanup") }
    }

    private func deleteTemporaryLocalProfilesIfPossible() {
        guard openProfileManagerIfPossible() else { return }
    ...
    private func findFirstTemporaryLocalProfile() -> TemporaryLocalProfile? {
        guard let button = firstTemporaryLocalProfileReplaceButton() else { return nil }
        ...
        return TemporaryLocalProfile(name: name, replaceIdentifier: button.identifier, manageIdentifier: "local-profile-manage-\(uuid)")
    }

    private func firstTemporaryLocalProfileReplaceButton() -> XCUIElement? { app.buttons.allElementsBoundByIndex.first { button in button.identifier.hasPrefix("local-profile-replace-") && isTemporaryLocalProfileName(button.label) } }
    ...
    private func deleteTemporaryLocalProfile(_ profile: TemporaryLocalProfile) {
        ...
        guard manageButton.waitForExistence(timeout: 2), waitForElementEnabled(manageButton, timeout: 2) else {
        ...
        let removed = waitUntil(timeout: 2) { !self.app.buttons[profile.replaceIdentifier].exists }
        XCTAssertTrue(removed, "Local profile \(profile.name) remained visible after cleanup")
    }

    private func isTemporaryLocalProfileName(_ name: String) -> Bool { temporaryLocalProfileNamePrefixes.contains { name.hasPrefix($0) } }
