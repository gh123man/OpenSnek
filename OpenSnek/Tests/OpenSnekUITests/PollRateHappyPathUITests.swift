...
final class PollRateHappyPathUITests: OpenSnekHardwareUITestCase {
    ...
    override func restoreHardwareStateIfNeeded() {
        guard let restore = pollRateToRestore else { return }
        _ = clickPollRateOption(restore, picker: app.descendants(matching: .any)["poll-rate-picker"])
        _ = waitForEvent(named: "applyEnd", timeout: 2, matching: { $0.patch?.pollRate == restore && expectedScope.matches($0.scope) })
    }
    ...
    func testChangingPollRateAppliesExpectedUSBCommandAndState() throws {
        ...
        let hydratedState = try XCTUnwrap(latestExpectedDeviceState(), "Expected a real readState event for \(expectedScope.description)")
        ...
        let clickStartedAt = try XCTUnwrap(clickPollRateOption(targetPollRate, picker: pollRatePicker), "Could not click \(targetPollRate) Hz")

        let applyEnd = try XCTUnwrap(waitForEvent(named: "applyEnd", timeout: 2, matching: { event in event.patch?.pollRate == targetPollRate && expectedScope.matches(event.scope) }), "Poll-rate apply to \(targetPollRate) Hz did not complete within 2s")
        ...
        let applyStart = try XCTUnwrap(events.first { $0.name == "applyStart" && $0.patch?.pollRate == targetPollRate && expectedScope.matches($0.scope) && $0.timestamp >= clickStartedAt.timeIntervalSince1970 - 0.1 }, "App did not record applyStart for \(targetPollRate) Hz")
        XCTAssertEqual(applyStart.patch?.pollRate, targetPollRate)
        assertExpectedScope(applyStart.scope, context: "applyStart")

        let command = try XCTUnwrap(
            events.first { $0.name == "usbCommand" && $0.timestamp >= applyStart.timestamp && $0.command?.name == "usbSetPollRate" && $0.command?.args == expectedArgs && expectedScope.matches($0.scope) }?.command, "App did not record the expected real USB poll-rate command for \(targetPollRate) Hz")
