...
final class AppStateProfileSwitchRegressionTests: XCTestCase {
    func testMappedProfileSwitchProjectsActiveSnapshotDPIIntoSelectedState() async throws {
        ...
        let travelSnapshot = makeProfileSwitchSnapshot(profileID: 2, identifier: travelIdentifier, name: "V3 Pro Travel", dpiValues: [600, 30_000], activeStage: 1)
        ...
        let selectedState = await MainActor.run {
            (
                appState.deviceStore.state?.active_onboard_profile, appState.deviceStore.state?.dpi?.x, appState.deviceStore.state?.dpi_stages.active_stage, appState.deviceStore.state?.dpi_stages.values, appState.editorStore.editableStageCount,
    ...
    func testConnectHydrationDoesNotReplaceReducedActiveProfileWithRawPaddedDPIStages() async throws {
        ...
        let baseSnapshot = makeProfileSwitchSnapshot(profileID: 1, identifier: UUID(), name: "Base", dpiValues: [800, 1600, 3200, 6400, 12_000], activeStage: 0)
        let workSnapshot = makeProfileSwitchSnapshot(profileID: 2, identifier: UUID(), name: "Work", dpiValues: [600, 30_000], activeStage: 1)
        let backend = makeProfileSwitchBackend(device: device, activeProfile: 2, dpiValues: [600, 30_000, 8000, 12_000, 16_000])
...
private func makeProfileSwitchBackend(device: MouseDevice, activeProfile: Int, dpiValues: [Int]) -> AppStateRefactorStubBackend {
...
private func makeProfileSwitchInventory(activeProfile: Int, maxProfileID: Int, snapshots: [OnboardProfileSnapshot]) -> OnboardProfileInventory {
    OnboardProfileInventory(
        activeProfileID: activeProfile, maxProfileID: maxProfileID, assignedProfileIDs: snapshots.map(\.profileID).sorted(),
        profiles: snapshots.map { snapshot in OnboardProfileSummary(profileID: snapshot.profileID, metadata: snapshot.metadata, isAssigned: true, isActive: snapshot.profileID == activeProfile, isBaseProfile: snapshot.profileID == 1) })
}

private func makeProfileSwitchSnapshot(profileID: Int, identifier: UUID, name: String, dpiValues: [Int], activeStage: Int) -> OnboardProfileSnapshot {
...
private func makeProfileSwitchTravelProfile(sourceDevice: MouseDevice) -> OpenSnekLocalProfile {
    DevicePreferenceStore().upsertOpenSnekLocalProfile(
        ...
        content: OpenSnekLocalProfileContent(
            ...
            staticColorByLEDID: [4: RGBPatch(r: 11, g: 22, b: 33)], scrollMode: 1, scrollAcceleration: true, scrollSmartReel: true), onboardIdentifier: UUID(), device: sourceDevice)
}
