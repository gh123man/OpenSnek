...
final class RemoteServiceSnapshotTests: XCTestCase {
    ...
    func testRemoteServiceDeviceStateUpdateAppliesWithoutWaitingForSnapshot() async {
        let device = makeSnapshotDevice(
            id: "remote-delta-device",
            productName: "Remote Delta Mouse",
            transport: .bluetooth,
            serial: "REMOTE-DELTA",
            locationID: 5,
            profile: .basiliskV3XHyperspeed
        ...
        let backend = RemoteDeltaTestBackend(device: device, initialState: initialState)
        let appState = await MainActor.run {
    ...
    func testRemoteServiceTransportStatusUpdateShowsStreamActiveBeforeFirstDpiEvent() async {
        let device = makeSnapshotDevice(
            id: "remote-stream-status-device",
            productName: "Remote Stream Mouse",
            transport: .bluetooth,
            serial: "REMOTE-STREAM",
            locationID: 6,
            profile: .basiliskV3XHyperspeed
        )
        let initialState = makeSnapshotState(
        ...
        let backend = RemoteDeltaTestBackend(
            device: device,
            initialState: initialState,
            initialTransportStatus: .listening
        )
        let appState = await MainActor.run {
            AppState(launchRole: .app, backend: backend, autoStart: false)
        }
        ...
        await Task.yield()
        await appState.deviceStore.refreshDevices()
        await backend.emitTransportStatusUpdate(.streamActive, deviceID: device.id)
        try? await Task.sleep(nanoseconds: 50_000_000)

        let tooltip = await MainActor.run { appState.deviceStore.currentDeviceConnectionTooltip }
        XCTAssertTrue(tooltip?.contains("Real-time HID: HID stream active") == true)
    }
}
...
private actor RemoteDeltaTestBackend: DeviceBackend {
    ...
    func emitStateUpdate(_ nextState: MouseState, deviceID: String, updatedAt: Date = Date()) {
        state = nextState
    ...
    func emitTransportStatusUpdate(
        _ nextStatus: DpiUpdateTransportStatus,
        deviceID: String,
        updatedAt: Date = Date()
    ) {
        transportStatus = nextStatus
        stateUpdateStreamPair.continuation.yield(
            .dpiTransportStatus(deviceID: deviceID, status: nextStatus, updatedAt: updatedAt)
        )
    }
}
