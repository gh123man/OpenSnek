...
final class AppStateMultiDeviceSelectionTests: XCTestCase {
    func testSelectingVisibleDeviceWithoutCachedStateStartsImmediateRefresh() async throws {
        ...
        let backend = DeviceListUpdatingStubBackend(devices: [alphaDevice], stateByDeviceID: [alphaDevice.id: makeTestState(device: alphaDevice, connection: "usb", batteryPercent: 81, dpiValues: [1200, 2400, 3600], activeStage: 0)])
        let appState = await MainActor.run { AppState(launchRole: .app, backend: backend, autoStart: false) }

        await appState.deviceStore.refreshDevices()
        await backend.setState(makeTestState(device: betaDevice, connection: "bluetooth", batteryPercent: 72, dpiValues: [3200, 4800, 6400], activeStage: 1), for: betaDevice.id)
        ...
        try await waitForAppStateCondition { await backend.readCount(for: betaDevice.id) == 1 }
    ...
    func testSelectedUnavailableDeviceWithCachedStateDisconnectsAndLocksControls() async {
        let usbDevice = makeTestDevice(id: "usb-dongle", productName: "Zeta Mouse", identity: MultiDeviceTestIdentity(transport: .usb, serial: "USB-IDLE", locationID: 2), profile: .basiliskV3Pro)
        let initialState = makeTestState(device: usbDevice, connection: "usb", batteryPercent: 72, dpiValues: [800, 900, 2000, 1100, 1200], activeStage: 2)
        let backend = DisconnectingMultiDeviceStubBackend(devices: [usbDevice], stateByDeviceID: [usbDevice.id: initialState])
        let appState = await MainActor.run { AppState(launchRole: .app, backend: backend, autoStart: false) }
        ...
        await MainActor.run { _ = appState.deviceController.applyDeviceList([], source: "test") }
    ...
    func testSelectedUnavailableDeviceRecoveryStartsImmediateRefresh() async throws {
        let usbDevice = makeTestDevice(id: "usb-recovery", productName: "Zeta Mouse", identity: MultiDeviceTestIdentity(transport: .usb, serial: "USB-RECOVER", locationID: 2), profile: .basiliskV3Pro)
        let restoredState = makeTestState(device: usbDevice, connection: "usb", batteryPercent: 72, dpiValues: [800, 900, 2000, 1100, 1200], activeStage: 2)
        let backend = DisconnectingMultiDeviceStubBackend(devices: [usbDevice], stateByDeviceID: [usbDevice.id: restoredState])
        let appState = await MainActor.run { AppState(launchRole: .app, backend: backend, autoStart: false) }
        ...
        try await waitForAppStateCondition { await MainActor.run { appState.deviceStore.state?.dpi?.x == 2000 } }
    ...
    func testDiagnosticsExposePollingVsRealtimeHIDAndDisableControlsWhenDisconnected() async throws {
        let usbDevice = makeTestDevice(id: "usb-diagnostics", productName: "Zeta Mouse", identity: MultiDeviceTestIdentity(transport: .usb, serial: "USB-DIAG", locationID: 3), profile: .basiliskV3Pro)
        let initialState = makeTestState(device: usbDevice, connection: "usb", batteryPercent: 68, dpiValues: [800, 1600, 3200], activeStage: 1)
        let backend = DeviceListUpdatingStubBackend(devices: [usbDevice], stateByDeviceID: [usbDevice.id: initialState], shouldUseFastDPIPolling: true)
        let appState = await MainActor.run { AppState(launchRole: .app, backend: backend, autoStart: false) }
        ...
        try await waitForAppStateCondition { await MainActor.run { !appState.deviceStore.selectedDeviceControlsEnabled } }
    ...
    func testConnectedStatusPillStaysGreenWhileListeningForFirstHIDEvent() async {
        let bluetoothDevice = makeTestDevice(id: "bt-fallback", productName: "Basilisk V3 Pro", identity: MultiDeviceTestIdentity(transport: .bluetooth, serial: "BT-FALLBACK", locationID: 4), profile: .basiliskV3Pro)
        let initialState = makeTestState(device: bluetoothDevice, connection: "bluetooth", batteryPercent: 74, dpiValues: [800, 1600, 3200], activeStage: 1)
        let backend = DeviceListUpdatingStubBackend(devices: [bluetoothDevice], stateByDeviceID: [bluetoothDevice.id: initialState], shouldUseFastDPIPolling: true, dpiUpdateTransportStatus: .listening)
        let appState = await MainActor.run { AppState(launchRole: .app, backend: backend, autoStart: false) }
    ...
    func testConnectedStatusPillWarnsWhenRealtimeHIDFallsBack() async {
        let bluetoothDevice = makeTestDevice(id: "bt-warning", productName: "Basilisk V3 Pro", identity: MultiDeviceTestIdentity(transport: .bluetooth, serial: "BT-WARNING", locationID: 5), profile: .basiliskV3Pro)
        let initialState = makeTestState(device: bluetoothDevice, connection: "bluetooth", batteryPercent: 71, dpiValues: [800, 1600, 3200], activeStage: 1)
        let backend = DeviceListUpdatingStubBackend(devices: [bluetoothDevice], stateByDeviceID: [bluetoothDevice.id: initialState], shouldUseFastDPIPolling: true, dpiUpdateTransportStatus: .pollingFallback, hidAccessAuthorization: .denied)
        let appState = await MainActor.run { AppState(launchRole: .app, backend: backend, autoStart: false) }
    ...
    func testConnectionDiagnosticsRevisionUpdatesWhenTransportStatusChanges() async {
        let bluetoothDevice = makeTestDevice(id: "bt-revision", productName: "Basilisk V3 Pro", identity: MultiDeviceTestIdentity(transport: .bluetooth, serial: "BT-REVISION", locationID: 6), profile: .basiliskV3Pro)
        let initialState = makeTestState(device: bluetoothDevice, connection: "bluetooth", batteryPercent: 69, dpiValues: [800, 1600, 3200], activeStage: 1)
        let backend = DeviceListUpdatingStubBackend(devices: [bluetoothDevice], stateByDeviceID: [bluetoothDevice.id: initialState], shouldUseFastDPIPolling: true, dpiUpdateTransportStatus: .pollingFallback)
        let appState = await MainActor.run { AppState(launchRole: .app, backend: backend, autoStart: false) }
    ...
    func testSelectedStateRefreshInvalidatesConnectionDiagnosticsAndUnlocksControls() async {
        let usbDevice = makeTestDevice(id: "usb-refresh-unlocks-controls", productName: "Basilisk V3 Pro", identity: MultiDeviceTestIdentity(transport: .usb, serial: "USB-UNLOCKS", locationID: 7), profile: .basiliskV3Pro)
        let refreshedState = makeTestState(device: usbDevice, connection: "usb", batteryPercent: 72, dpiValues: [800, 1600, 3200], activeStage: 1)
        let backend = DeviceListUpdatingStubBackend(devices: [usbDevice], stateByDeviceID: [usbDevice.id: refreshedState])
        let appState = await MainActor.run { AppState(launchRole: .app, backend: backend, autoStart: false) }
        await MainActor.run {
    ...
    func testFastDpiPollingDoesNotDowngradeListeningStatusToFallback() async {
        let bluetoothDevice = makeTestDevice(id: "bt-listening-fast", productName: "Basilisk V3 Pro", identity: MultiDeviceTestIdentity(transport: .bluetooth, serial: "BT-LISTENING-FAST", locationID: 7), profile: .basiliskV3Pro)
        let initialState = makeTestState(device: bluetoothDevice, connection: "bluetooth", batteryPercent: 66, dpiValues: [800, 1600, 3200], activeStage: 1)
        let backend = DeviceListUpdatingStubBackend(devices: [bluetoothDevice], stateByDeviceID: [bluetoothDevice.id: initialState], shouldUseFastDPIPolling: true, dpiUpdateTransportStatus: .listening)
        let appState = await MainActor.run { AppState(launchRole: .app, backend: backend, autoStart: false) }
    ...
    func testFreshPassiveHeartbeatKeepsBluetoothConnectedAfterIdleTelemetryGap() async {
        let bluetoothDevice = makeTestDevice(id: "bt-passive-heartbeat", productName: "Basilisk V3 Pro", identity: MultiDeviceTestIdentity(transport: .bluetooth, serial: "BT-PASSIVE-HEARTBEAT", locationID: 8), profile: .basiliskV3Pro)
        let initialState = makeTestState(device: bluetoothDevice, connection: "bluetooth", batteryPercent: 73, dpiValues: [800, 1600, 3200], activeStage: 1)
        let backend = DeviceListUpdatingStubBackend(devices: [bluetoothDevice], stateByDeviceID: [bluetoothDevice.id: initialState], shouldUseFastDPIPolling: false, dpiUpdateTransportStatus: .streamActive)
        let appState = await MainActor.run { AppState(launchRole: .app, backend: backend, autoStart: false) }

        await appState.deviceStore.refreshDevices()

        await MainActor.run {
            appState.deviceController.storeState(initialState, for: bluetoothDevice.id, updatedAt: Date().addingTimeInterval(-30))
            appState.deviceController.applyBackendDpiTransportStatusUpdate(deviceID: bluetoothDevice.id, status: .streamActive, updatedAt: Date())
        }
    ...
    func testBackendDeviceListUpdateRemovesDisconnectedDeviceImmediately() async throws {
        let bluetoothDevice = makeTestDevice(id: "bt-device", productName: "Alpha Mouse", identity: MultiDeviceTestIdentity(transport: .bluetooth, serial: "BT-ONE", locationID: 1), profile: .basiliskV3XHyperspeed)
        let backend = DeviceListUpdatingStubBackend(devices: [bluetoothDevice], stateByDeviceID: [bluetoothDevice.id: makeTestState(device: bluetoothDevice, connection: "bluetooth", batteryPercent: 72, dpiValues: [800, 1600, 2400], activeStage: 1)])
        let appState = await MainActor.run { AppState(launchRole: .app, backend: backend, autoStart: false) }

        await appState.deviceStore.refreshDevices()
        await backend.emitDeviceListUpdate([])

        try await waitForAppStateCondition { await MainActor.run { appState.deviceStore.devices.isEmpty } }
    ...
    func testBackendDeviceListUpdateRefreshesStateForReconnectWithStableDeviceID() async throws {
        let usbDevice = makeTestDevice(id: "usb-reconnect", productName: "Alpha Mouse", identity: MultiDeviceTestIdentity(transport: .usb, serial: "USB-ONE", locationID: 1), profile: .basiliskV3Pro)
        let initialState = makeTestState(device: usbDevice, connection: "usb", batteryPercent: 81, dpiValues: [800, 1600, 3200], activeStage: 0)
        let refreshedState = makeTestState(device: usbDevice, connection: "usb", batteryPercent: 81, dpiValues: [800, 1600, 3200], activeStage: 2)
        ...
        try await waitForAppStateCondition { await MainActor.run { appState.deviceStore.state?.dpi?.x == 3200 } }
    ...
    func testUSBReconnectWithNewDeviceIDSeedsPreviousState() async throws {
        let originalDevice = makeTestDevice(id: "usb-reconnect-original", productName: "Alpha Mouse", identity: MultiDeviceTestIdentity(transport: .usb, serial: "USB-RECONNECT-SERIAL", locationID: 1), profile: .basiliskV335K)
        let replacementDevice = makeTestDevice(id: "usb-reconnect-replacement", productName: "Alpha Mouse", identity: MultiDeviceTestIdentity(transport: .usb, serial: "USB-RECONNECT-SERIAL", locationID: 2), profile: .basiliskV335K)
        let originalState = makeTestState(device: originalDevice, connection: "usb", batteryPercent: 81, dpiValues: [800, 1600, 3200], activeStage: 0)
        let replacementState = makeTestState(device: replacementDevice, connection: "usb", batteryPercent: 81, dpiValues: [800, 1600, 3200], activeStage: 2)
        let backend = DeviceListUpdatingStubBackend(devices: [originalDevice], stateByDeviceID: [originalDevice.id: originalState, replacementDevice.id: replacementState])
        let appState = await MainActor.run { AppState(launchRole: .app, backend: backend, autoStart: false) }

        await appState.deviceStore.refreshDevices()
        await backend.setTransientReadFailures(["USB device telemetry unavailable. Feature-report interface did not return usable responses."], for: replacementDevice.id)
        await backend.emitDeviceListUpdate([replacementDevice])

        try await waitForAppStateCondition { await MainActor.run { appState.deviceStore.selectedDeviceID == replacementDevice.id } }
    ...
    func testBackendDeviceListUpdateRearmsSettingsRestoreForStableReconnect() async throws {
        let usbDevice = makeTestDevice(id: "usb-lighting-reconnect", productName: "Alpha Mouse", identity: MultiDeviceTestIdentity(transport: .usb, serial: "USB-LIGHTING-RECONNECT", locationID: 1), profile: .basiliskV3XHyperspeed)
        ...
        try await waitForAppStateCondition { await backend.applyCount() >= initialApplyCount + 1 }

        let patches = await backend.recordedPatches()
        let restorePatches = patches.filter { $0.pollRate == 500 }
        let restoredPrimaryRs = restorePatches.compactMap { patch in patch.ledRGB?.r ?? patch.lightingEffect?.primary.r }
    ...
    func testNonSelectedReconnectSettingsRestoreDoesNotChangeSelection() async throws {
        let alphaDevice = makeTestDevice(id: "alpha-selected", productName: "Alpha Mouse", identity: MultiDeviceTestIdentity(transport: .usb, serial: "ALPHA-SELECTED", locationID: 1), profile: .basiliskV3XHyperspeed)
        let betaDevice = makeTestDevice(id: "beta-restore", productName: "Beta Mouse", identity: MultiDeviceTestIdentity(transport: .usb, serial: "BETA-RESTORE", locationID: 2), profile: .basiliskV3XHyperspeed)
        ...
        let backend = DeviceListUpdatingStubBackend(
            ...
            stateByDeviceID: [
                alphaDevice.id: makeTestState(device: alphaDevice, connection: "usb", batteryPercent: 81, dpiValues: [800, 1600, 3200], activeStage: 0), betaDevice.id: makeTestState(device: betaDevice, connection: "usb", batteryPercent: 79, dpiValues: [800, 1600, 3200], activeStage: 1)
            ])
        let appState = await MainActor.run { AppState(launchRole: .app, backend: backend, autoStart: false) }
