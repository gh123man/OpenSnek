...
@MainActor final class WindowChromeConfiguratorTests: XCTestCase {
    ...
    func testPersistFrameSkipsWritesWhenDeveloperRememberWindowSizeIsDisabled() {
        ...
        let window = NSWindow(contentRect: NSRect(x: 40, y: 50, width: 1210, height: 780), styleMask: [.titled, .closable, .miniaturizable, .resizable], backing: .buffered, defer: false)

        WindowChromeConfigurator.persistFrame(window, autosaveName: autosaveName, defaults: defaults)
    ...
    func testConfigureRestoresPersistedFrameFromDefaults() {
        ...
        let persistedWindow = NSWindow(contentRect: NSRect(x: 120, y: 80, width: 1330, height: 840), styleMask: [.titled, .closable, .miniaturizable, .resizable], backing: .buffered, defer: false)
        ...
        let restored = WindowChromeConfigurator.restorePersistedFrameIfNeeded(window, autosaveName: autosaveName, defaults: defaults, visibleScreenFrames: [NSRect(x: 0, y: 0, width: 1600, height: 1000)])
    ...
    func testRestorePersistedFrameShrinksFrameToFitVisibleScreen() {
        ...
        let restored = WindowChromeConfigurator.restorePersistedFrameIfNeeded(window, autosaveName: autosaveName, defaults: defaults, visibleScreenFrames: [NSRect(x: 0, y: 0, width: 1440, height: 900)])
    ...
    func testNormalizedPersistedFrameRepositionsCurrentWindowFrame() {
        let normalized = WindowChromeConfigurator.normalizedPersistedFrame(NSRect(x: -900, y: -200, width: 1200, height: 828), visibleScreenFrames: [NSRect(x: 0, y: 0, width: 1440, height: 900)])
