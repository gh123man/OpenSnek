...
final class AppStateOnboardMutationTests: XCTestCase {
    func testCreatingOnboardProfileCanCopyExistingSlot() async throws {
        let device = makeRefactorTestDevice(id: "onboard-copy-create-device", transport: .usb, serial: "ONBOARD-COPY-\(UUID().uuidString)", onboardProfileCount: 5, profileID: .basiliskV3Pro)
        ...
        let sourceSnapshot = makeRefactorOnboardProfileSnapshot(
            profileID: 3, name: "Stored 3", dpiValues: [3200, 6400], buttonBindings: [4: copiedBinding], brightnessByLEDID: [1: 210, 4: 180, 10: 150], staticColorByLEDID: [1: RGBPatch(r: 10, g: 20, b: 30), 4: RGBPatch(r: 40, g: 50, b: 60), 10: RGBPatch(r: 70, g: 80, b: 90)])
        ...
        await appState.editorStore.createOnboardProfile(name: "Copied", targetProfileID: 2, copyFromProfileID: 3)

        try await waitForRefactorCondition { await MainActor.run { appState.editorStore.onboardProfileSummaries.first(where: { $0.profileID == 2 })?.displayName == "Copied" } }
    ...
    func testRenamingOnboardProfileUpdatesCachedSummaryWithoutRefreshingInventory() async throws {
        let device = makeRefactorTestDevice(id: "onboard-rename-device", transport: .usb, serial: "ONBOARD-RENAME-\(UUID().uuidString)", onboardProfileCount: 5, profileID: .basiliskV3Pro)
        ...
        try await waitForRefactorCondition { await MainActor.run { appState.editorStore.selectedOnboardProfileName == "Renamed" && appState.editorStore.onboardProfileSummaries.first(where: { $0.profileID == 2 })?.displayName == "Renamed" } }
    ...
    func testMetadataObjectOnboardProfileRenamePreservesLoadedSnapshotForNextEdit() async throws {
        let device = makeRefactorTestDevice(id: "onboard-rename-metadata-object-device", transport: .usb, serial: "ONBOARD-RENAME-METADATA-OBJECT-\(UUID().uuidString)", onboardProfileCount: 5, profileID: .basiliskV3Pro)
        let backend = AppStateRefactorStubBackend(
            devices: [device],
            stateByDeviceID: [device.id: makeRefactorTestState(device: device, telemetry: RefactorTestStateTelemetry(connection: "usb", batteryPercent: 74, dpiValues: [800, 1600, 3200], activeStage: 0), options: RefactorTestStateOptions(activeOnboardProfile: 1, onboardProfileCount: 5))])
        await backend.setOnboardInventory(
        ...
        await backend.setOnboardSnapshot(
            OnboardProfileSnapshot(
                profileID: 2, metadata: OnboardProfileMetadata(name: "Stored 2"), dpi: OnboardDPIProfileSnapshot(scalar: DpiPair(x: 1200, y: 1200), activeStage: 0, pairs: [DpiPair(x: 1200, y: 1200), DpiPair(x: 2400, y: 2400)], stageIDs: [0x21, 0x22], marker: 0xA5),
                buttonBindings: [4: ButtonBindingDraft(kind: .rightClick, hidKey: 4, turboEnabled: false, turboRate: 0x8E)], brightnessByLEDID: [1: 64]), forDeviceID: device.id)
        ...
        await MainActor.run {
            ...
            appState.editorStore.editableStagePairs = [DpiPair(x: 1400, y: 1400), DpiPair(x: 2600, y: 2600), DpiPair(x: 3200, y: 3200), DpiPair(x: 6400, y: 6400), DpiPair(x: 12000, y: 12000)]
    ...
    func testRenamedOnboardProfilePreservesProjectedNameAcrossStaleInventoryRefresh() async throws {
        let device = makeRefactorTestDevice(id: "onboard-rename-stale-inventory-device", transport: .usb, serial: "ONBOARD-RENAME-STALE-\(UUID().uuidString)", onboardProfileCount: 5, profileID: .basiliskV3Pro)
        let backend = AppStateRefactorStubBackend(
            devices: [device],
            stateByDeviceID: [device.id: makeRefactorTestState(device: device, telemetry: RefactorTestStateTelemetry(connection: "usb", batteryPercent: 74, dpiValues: [800, 1600, 3200], activeStage: 0), options: RefactorTestStateOptions(activeOnboardProfile: 1, onboardProfileCount: 5))])
        let staleInventory = OnboardProfileInventory(activeProfileID: 1, maxProfileID: 5, assignedProfileIDs: [1, 2], profiles: [makeRefactorOnboardProfileSummary(profileID: 1, name: "Base", isActive: true), makeRefactorOnboardProfileSummary(profileID: 2, name: "Stored 2", isActive: false)])
    ...
    func testProjectedOnboardProfileNameDoesNotResurrectUnassignedSlot() async throws {
        let device = makeRefactorTestDevice(id: "onboard-rename-unassigned-refresh-device", transport: .usb, serial: "ONBOARD-RENAME-UNASSIGNED-\(UUID().uuidString)", onboardProfileCount: 5, profileID: .basiliskV3Pro)
        let backend = AppStateRefactorStubBackend(
            devices: [device],
            stateByDeviceID: [device.id: makeRefactorTestState(device: device, telemetry: RefactorTestStateTelemetry(connection: "usb", batteryPercent: 74, dpiValues: [800, 1600, 3200], activeStage: 0), options: RefactorTestStateOptions(activeOnboardProfile: 1, onboardProfileCount: 5))])
        await backend.setOnboardInventory(
        ...
        try await waitForRefactorCondition {
            await MainActor.run {
                ...
                return appState.editorStore.selectedOnboardProfileID == 1 && slot?.isAssigned == false && slot?.metadata == nil
            }
        }
    }

    func testDeletingActiveOnboardProfileActivatesNextAssignedSlot() async throws {
        let device = makeRefactorTestDevice(id: "onboard-delete-active-device", transport: .usb, serial: "ONBOARD-DELETE-ACTIVE-\(UUID().uuidString)", onboardProfileCount: 5, profileID: .basiliskV3Pro)
        let backend = AppStateRefactorStubBackend(
            devices: [device],
        ...
        try await waitForRefactorCondition {
            await MainActor.run {
                appState.editorStore.selectedOnboardProfileID == 3 && appState.editorStore.onboardProfileSummaries.first(where: { $0.profileID == 2 })?.isAssigned == false && appState.editorStore.onboardProfileSummaries.first(where: { $0.profileID == 3 })?.isActive == true
                    && appState.deviceStore.state?.active_onboard_profile == 3 && appState.editorStore.stageValue(0) == 3200
            }
        }
    ...
    func testSelectedOnboardProfileDpiEditUpdatesActiveOnboardProfile() async throws {
        let device = makeRefactorTestDevice(id: "onboard-inactive-edit-device", transport: .usb, serial: "ONBOARD-INACTIVE-\(UUID().uuidString)", onboardProfileCount: 5, profileID: .basiliskV3Pro)
        let backend = AppStateRefactorStubBackend(
            devices: [device],
            stateByDeviceID: [device.id: makeRefactorTestState(device: device, telemetry: RefactorTestStateTelemetry(connection: "usb", batteryPercent: 74, dpiValues: [800, 1600, 3200], activeStage: 0), options: RefactorTestStateOptions(activeOnboardProfile: 1, onboardProfileCount: 5))])
        await backend.setOnboardInventory(
        ...
        await appState.editorStore.refreshOnboardProfiles()
        await appState.editorStore.selectOnboardProfile(2)
        try await waitForRefactorCondition { await MainActor.run { appState.editorStore.onboardProfileSummaries.first(where: { $0.profileID == 2 })?.isActive == true && appState.deviceStore.state?.active_onboard_profile == 2 } }
        await MainActor.run {
            appState.editorStore.editableStageCount = 2
    ...
    func testBluetoothOnboardProfileDeviceEditorChangesUpdateStoredProfile() async throws {
        let device = makeRefactorTestDevice(id: "onboard-bt-stored-editor-device", transport: .bluetooth, serial: "ONBOARD-BT-STORED-EDITOR-\(UUID().uuidString)", onboardProfileCount: 5, profileID: .basiliskV3Pro)
        let backend = AppStateRefactorStubBackend(
            devices: [device],
            stateByDeviceID: [
        ...
        await backend.setOnboardInventory(
            OnboardProfileInventory(activeProfileID: 1, maxProfileID: 5, assignedProfileIDs: [1, 2], profiles: [makeRefactorOnboardProfileSummary(profileID: 1, name: "Base", isActive: true), makeRefactorOnboardProfileSummary(profileID: 2, name: "Stored 2", isActive: false)]), forDeviceID: device.id)
        await backend.setOnboardSnapshot(
            makeRefactorOnboardProfileSnapshot(profileID: 1, name: "Base", dpiValues: [400, 800, 1300, 1600, 6400], brightnessByLEDID: [1: 128, 4: 128, 10: 128], staticColorByLEDID: [1: RGBPatch(r: 0, g: 0, b: 255), 4: RGBPatch(r: 0, g: 0, b: 255), 10: RGBPatch(r: 0, g: 0, b: 255)]),
        ...
        await MainActor.run {
            ...
            appState.editorStore.editableStagePairs = [DpiPair(x: 500, y: 500), DpiPair(x: 900, y: 900), DpiPair(x: 1400, y: 1400), DpiPair(x: 1600, y: 1600), DpiPair(x: 6400, y: 6400)]
        ...
        try await waitForRefactorCondition {
            let updates = await backend.recordedOnboardUpdates()
            return updates.contains { update in update.profileID == 1 && update.mutation.staticColorByLEDID?[1] == RGBPatch(r: 255, g: 0, b: 0) }
        }
        ...
        try await waitForRefactorCondition {
            let updates = await backend.recordedOnboardUpdates()
            return updates.contains { update in update.profileID == 1 && update.mutation.brightnessByLEDID?[1] == 200 }
        }

        await MainActor.run { appState.editorStore.updateButtonBindingKind(slot: 4, kind: .mouseForward) }
        try await waitForRefactorCondition { await backend.recordedOnboardUpdates().count == 4 }
    ...
    func testBluetoothOnboardProfileButtonEditUpdatesSelectedStoredProfile() async throws {
        let device = makeRefactorTestDevice(id: "onboard-bt-selected-button-device", transport: .bluetooth, serial: "ONBOARD-BT-SELECTED-BUTTON-\(UUID().uuidString)", onboardProfileCount: 5, profileID: .basiliskV3Pro)
        let backend = AppStateRefactorStubBackend(
            devices: [device],
            stateByDeviceID: [device.id: makeRefactorTestState(device: device, telemetry: RefactorTestStateTelemetry(connection: "bluetooth", batteryPercent: 74, dpiValues: [400, 1200, 1300], activeStage: 0), options: RefactorTestStateOptions(activeOnboardProfile: 1, onboardProfileCount: 5))])
        let baseBinding = ButtonBindingDraft(kind: .rightClick, hidKey: 4, turboEnabled: false, turboRate: 0x8E)
        await backend.setOnboardInventory(
            OnboardProfileInventory(activeProfileID: 1, maxProfileID: 5, assignedProfileIDs: [1, 2], profiles: [makeRefactorOnboardProfileSummary(profileID: 1, name: "Base", isActive: true), makeRefactorOnboardProfileSummary(profileID: 2, name: "Stored 2", isActive: false)]), forDeviceID: device.id)
        await backend.setOnboardSnapshot(makeRefactorOnboardProfileSnapshot(profileID: 2, name: "Stored 2", buttonBindings: [4: baseBinding]), forDeviceID: device.id)
        ...
        try await waitForRefactorCondition { await MainActor.run { appState.editorStore.selectedOnboardProfileID == 2 && appState.editorStore.selectedOnboardProfileIsActive && appState.editorStore.buttonBindingKind(for: 4) == .rightClick } }
        try await waitForRefactorCondition { await backend.onboardButtonReadCount(deviceID: device.id, profileID: 2) >= 1 }
    ...
    func testBluetoothOnboardProfileSwitchHydratesProfileSpecificButtonBindings() async throws {
        let device = makeRefactorTestDevice(id: "onboard-bt-switch-button-device", transport: .bluetooth, serial: "ONBOARD-BT-SWITCH-BUTTON-\(UUID().uuidString)", onboardProfileCount: 5, profileID: .basiliskV3Pro)
        let backend = AppStateRefactorStubBackend(
            devices: [device],
            stateByDeviceID: [device.id: makeRefactorTestState(device: device, telemetry: RefactorTestStateTelemetry(connection: "bluetooth", batteryPercent: 74, dpiValues: [400, 1200, 1300], activeStage: 0), options: RefactorTestStateOptions(activeOnboardProfile: 1, onboardProfileCount: 5))])
        await backend.setOnboardInventory(
        ...
        await backend.setOnboardSnapshot(makeRefactorOnboardProfileSnapshot(profileID: 2, name: "Stored 2", buttonBindings: [4: ButtonBindingDraft(kind: .mouseBack, hidKey: 4, turboEnabled: false, turboRate: 0x8E)]), forDeviceID: device.id)
        await backend.setOnboardSnapshot(makeRefactorOnboardProfileSnapshot(profileID: 3, name: "Stored 3", buttonBindings: [4: ButtonBindingDraft(kind: .mouseForward, hidKey: 5, turboEnabled: false, turboRate: 0x8E)]), forDeviceID: device.id)
        ...
        await appState.editorStore.selectOnboardProfile(2)
        let profile2Kind = await MainActor.run { appState.editorStore.buttonBindingKind(for: 4) }

        await appState.editorStore.selectOnboardProfile(3)
        let profile3Kind = await MainActor.run { appState.editorStore.buttonBindingKind(for: 4) }
