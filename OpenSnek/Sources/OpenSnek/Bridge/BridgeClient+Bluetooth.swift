...
extension BridgeClient {
    ...
    func formatLightingZoneValues(_ values: [(UInt8, Int)]) -> String { values.map { ledID, value in "0x\(String(format: "%02x", ledID))=\(value)" }.joined(separator: ",") }

    func formatLightingZoneColors(_ values: [(UInt8, RGBPatch)]) -> String { values.map { ledID, color in "0x\(String(format: "%02x", ledID))=(\(color.r),\(color.g),\(color.b))" }.joined(separator: ",") }

    func btHex(_ data: Data) -> String { data.map { String(format: "%02x", $0) }.joined() }

    func btKeyLabel(_ key: BLEVendorProtocol.Key) -> String { btHex(Data(key.bytes)) }
    ...
    static func resolveBluetoothBatteryState(device: MouseDevice, vendorRaw: Int?, vendorStatus: Int?, usbFallback: (Int, Bool)?) -> BluetoothBatteryState {
        ...
        return BluetoothBatteryState(percent: vendorPercent ?? usbFallback?.0, charging: charging)
    }
    ...
    func btGetLightingValue(device: MouseDevice, ledIDs: [UInt8]? = nil) async throws -> Int? {
        if isBluetoothV3ProLightingDevice(device) {
            ...
            if values.contains(where: { $0.1 != first }) { AppLog.debug("Bridge", "btGetLightingValue zone-mismatch device=\(device.id) values=\(formatLightingZoneValues(values))") }
            return first
        }
    ...
    func btReadLightingColor(device: MouseDevice, ledID: UInt8) async throws -> RGBPatch? {
        ...
        guard let payload = BLEVendorProtocol.parsePayloadFrames(notifies: notifies, req: req) else {
            AppLog.debug("Bridge", "btReadLightingColor no-payload device=\(device.id) led=0x\(String(format: "%02x", ledID)) req=\(req) notifies=\(btNotifySummary(notifies))")
            return nil
        }
        let parsed = parseLightingRGB(payload: payload)
        if let parsed {
            AppLog.debug("Bridge", "btReadLightingColor device=\(device.id) led=0x\(String(format: "%02x", ledID)) rgb=(\(parsed.r),\(parsed.g),\(parsed.b))")
        } else {
            AppLog.debug("Bridge", "btReadLightingColor parse-failed device=\(device.id) led=0x\(String(format: "%02x", ledID)) payload=\(btHex(payload))")
        }
        return parsed
    }
    ...
    func readBluetoothState(device: MouseDevice, session: USBHIDControlSession?) async throws -> MouseState {
        ...
        if supportsMappedOnboardProfiles {
            btStages = lastStateByDeviceID[device.id].flatMap { state in
                ...
                return BLEVendorProtocol.DpiStagesRead(active: active, count: values.count, values: values, pairs: pairs, marker: 0x03)
            }
        } else {
            btStages =
                (try? await btGetDpiStages(device: device))
                ?? btDpiSnapshotByDeviceID[device.id].map { snapshot in BLEVendorProtocol.DpiStagesRead(active: snapshot.active, count: snapshot.count, values: Array(snapshot.slots.prefix(snapshot.count)), pairs: Array(snapshot.pairs.prefix(snapshot.count)), marker: snapshot.marker) }
        }
        let shouldPollGenericTelemetry = Self.shouldPollBluetoothGenericTelemetryForReadState(supportsMappedOnboardProfiles: supportsMappedOnboardProfiles)
        let batteryRaw: Int?
        ...
        let activeOnboardProfile = profile?.supportsMappedOnboardProfileCRUD == true ? (try? await btReadActiveOnboardProfileID(device: device)) ?? nil : nil

        let usbBatteryFallback = session.flatMap { try? getBattery($0, device) }
        let batteryState = Self.resolveBluetoothBatteryState(device: device, vendorRaw: batteryRaw, vendorStatus: batteryStatus, usbFallback: usbBatteryFallback)

        return MouseState(
            device: DeviceSummary(id: device.id, product_name: device.product_name, serial: device.serial, transport: device.transport, firmware: nil), connection: "Bluetooth", battery_percent: batteryState.percent, charging: batteryState.charging,
            dpi: {
                guard let active = btStages?.active, let values = btStages?.values, active >= 0, active < values.count else { return nil }
                if let pairs = btStages?.pairs, active < pairs.count { return pairs[active] }
                let value = values[active]
                return DpiPair(x: value, y: value)
            }(), dpi_stages: DpiStages(active_stage: btStages?.active, values: btStages?.values, pairs: btStages?.pairs), poll_rate: nil, sleep_timeout: sleepTimeout, device_mode: nil, active_onboard_profile: activeOnboardProfile,
            onboard_profile_count: profile?.supportsMappedOnboardProfileCRUD == true ? profile?.onboardProfileCount : nil, led_value: lighting, capabilities: Capabilities(dpi_stages: true, poll_rate: false, power_management: true, button_remap: true, lighting: supportsLighting))
    }

    nonisolated static func shouldPollBluetoothGenericTelemetryForReadState(supportsMappedOnboardProfiles: Bool) -> Bool { !supportsMappedOnboardProfiles }

    func buildBluetoothDeltaState(device: MouseDevice, includeDpi: Bool, includeLighting: Bool, includePower: Bool) async throws -> MouseState {
        let supportsLighting = device.showsLightingControls
        let btStages: BLEVendorProtocol.DpiStagesRead?
        if includeDpi { btStages = try await btGetDpiStages(device: device) } else { btStages = nil }

        let lighting: Int?
        if includeLighting, supportsLighting { lighting = try await btGetLightingValue(device: device) } else { lighting = nil }

        let sleepTimeout: Int?
        if includePower { sleepTimeout = try await btGetScalar(device: device, key: .powerTimeoutGet, size: 2) } else { sleepTimeout = nil }
        ...
        return MouseState(
            device: DeviceSummary(id: device.id, product_name: device.product_name, serial: device.serial, transport: device.transport, firmware: nil), connection: "Bluetooth", battery_percent: nil, charging: nil, dpi: dpiPair,
            dpi_stages: DpiStages(active_stage: btStages?.active, values: btStages?.values, pairs: btStages?.pairs), poll_rate: nil, sleep_timeout: sleepTimeout, device_mode: nil, led_value: lighting,
            capabilities: Capabilities(dpi_stages: true, poll_rate: false, power_management: true, button_remap: true, lighting: supportsLighting))
    }
    ...
    func btExchange(_ writes: [Data], timeout: TimeInterval = 0.8, device: MouseDevice? = nil, preferredPeripheralName: String? = nil) async throws -> [Data] {
        let start = Date()
        await btAcquireExchangeLock()
        defer { btReleaseExchangeLock() }

        let result = try await btVendorClient.run(writes: writes, timeout: timeout, preferredPeripheralName: preferredPeripheralName ?? device?.product_name)
        AppLog.debug("Bridge", "btExchange writes=\(writes.count) timeout=\(String(format: "%.2f", timeout))s " + "notifies=\(result.count) elapsed=\(String(format: "%.3f", Date().timeIntervalSince(start)))s")
        return result
    }
    ...
    func btAcquireExchangeLock() async {
        ...
        await withCheckedContinuation { continuation in btExchangeWaiters.append(continuation) }
    }
    ...
    func btGetScalar(device: MouseDevice, key: BLEVendorProtocol.Key, size: Int) async throws -> Int? {
        ...
        guard let payload = BLEVendorProtocol.parsePayloadFrames(notifies: notifies, req: req) else {
            AppLog.debug("Bridge", "btGetScalar no-payload device=\(device.id) key=\(btKeyLabel(key)) req=\(req) notifies=\(btNotifySummary(notifies))")
            return nil
        }
        guard payload.count >= size else {
            AppLog.debug("Bridge", "btGetScalar short-payload device=\(device.id) key=\(btKeyLabel(key)) req=\(req) " + "expected=\(size) actual=\(payload.count) payload=\(btHex(payload)) notifies=\(btNotifySummary(notifies))")
            return nil
        }
        return payload.prefix(size).enumerated().reduce(0) { partial, pair in partial | (Int(pair.element) << (pair.offset * 8)) }
    }
    ...
    func btAckSuccess(notifies: [Data], req: UInt8) -> Bool {
        for frame in notifies {
            ...
            if header.req == req { return header.status == 0x02 }
        }
        return false
    }
    ...
    func btGetDpiStages(device: MouseDevice) async throws -> BLEVendorProtocol.DpiStagesRead? {
        ...
        let notifies = try await btExchange([header], timeout: 0.6, device: device)
        guard let payload = BLEVendorProtocol.parsePayloadFrames(notifies: notifies, req: req) else {
            AppLog.debug("Bridge", "btGetDpiStages no-payload device=\(device.id) req=\(req) notifies=\(btNotifySummary(notifies))")
            return nil
        }
        guard let parsed = BLEVendorProtocol.parseDpiStages(blob: payload) else {
            AppLog.debug("Bridge", "btGetDpiStages parse-failed device=\(device.id) req=\(req) " + "payload=\(btHex(payload)) notifies=\(btNotifySummary(notifies))")
            return nil
        }

        let dpiRange = DeviceProfiles.dpiRange(for: device)
        guard !parsed.values.isEmpty, parsed.active >= 0, parsed.active < parsed.values.count, parsed.values.allSatisfy({ dpiRange.contains($0) }) else {
            AppLog.debug("Bridge", "btGetDpiStages ignored invalid payload device=\(device.id) values=\(parsed.values) active=\(parsed.active)")
            return nil
        }
        ...
        if var expected = btExpectedDpiByDeviceID[device.id] {
            ...
            } else if Date() < expected.expiresAt, expected.remainingMasks > 0, Self.shouldMaskBluetoothExpectedRead(parsedActive: parsed.active, parsedValues: parsed.values, parsedPairs: parsed.pairs, expected: expected) {
                expected.remainingMasks -= 1
                btExpectedDpiByDeviceID[device.id] = expected
                AppLog.debug("Bridge", "btGetDpiStages stale-read masked device=\(device.id) expectedActive=\(expected.active) expectedValues=\(expected.values) " + "actualActive=\(parsed.active) actualValues=\(parsedValues) remainingMasks=\(expected.remainingMasks)")
                return BLEVendorProtocol.DpiStagesRead(active: expected.active, count: expected.values.count, values: expected.values, pairs: expected.pairs, marker: parsed.marker)
            } else {
                btExpectedDpiByDeviceID[device.id] = nil
            }
        }

        if let snap = BLEVendorProtocol.parseDpiStageSnapshot(blob: payload) { btDpiSnapshotByDeviceID[device.id] = snap }
    ...
    func btGetDpiStageSnapshot(device: MouseDevice) async throws -> BLEVendorProtocol.DpiStageSnapshot? {
        ...
        let notifies = try await btExchange([header], timeout: 0.6, device: device)
        guard let payload = BLEVendorProtocol.parsePayloadFrames(notifies: notifies, req: req) else {
            AppLog.debug("Bridge", "btGetDpiStageSnapshot no-payload device=\(device.id) req=\(req) notifies=\(btNotifySummary(notifies))")
            return nil
        }
        guard let parsed = BLEVendorProtocol.parseDpiStageSnapshot(blob: payload) else {
            AppLog.debug("Bridge", "btGetDpiStageSnapshot parse-failed device=\(device.id) req=\(req) payload=\(btHex(payload)) " + "notifies=\(btNotifySummary(notifies))")
            return nil
        }
    ...
    func btSetDpiStages(device: MouseDevice, active: Int, values: [Int], pairs: [DpiPair]? = nil) async throws -> Bool {
        ...
        let currentPairs = current?.pairs ?? [DpiPair(x: 800, y: 800), DpiPair(x: 1600, y: 1600), DpiPair(x: 2400, y: 2400), DpiPair(x: 3200, y: 3200), DpiPair(x: 6400, y: 6400)]
        ...
        let mergedPairs = BLEVendorProtocol.mergedStagePairs(currentPairs: currentPairs, requestedCount: count, requestedPairs: requestedPairs)

        let payload = BLEVendorProtocol.buildDpiStagePayload(active: active, count: count, pairs: mergedPairs, marker: marker, stageIDs: currentStageIDs)
        let req = nextBTReq()
        ...
        if ok {
            ...
            btDpiSnapshotByDeviceID[device.id] = BLEVendorProtocol.DpiStageSnapshot(active: expectedActive, count: count, slots: mergedPairs.map(\.x), pairs: mergedPairs, stageIDs: currentStageIDs, marker: marker)
            btExpectedDpiByDeviceID[device.id] = BluetoothExpectedDpiState(
                active: expectedActive, values: expectedValues, pairs: expectedPairs, previousActive: previousSnapshot?.active, previousValues: previousSnapshot.map { Array($0.slots.prefix($0.count)) }, previousPairs: previousSnapshot.map { Array($0.pairs.prefix($0.count)) },
    ...
    func btSetLightingValue(device: MouseDevice, value: Int) async throws -> Bool {
        ...
        return try await btSetScalar(device: device, key: .lightingSet, value: clamped, size: 1, payloadLength: 0x01)
    }
    ...
    func btSetLightingRGB(device: MouseDevice, r: Int, g: Int, b: Int, ledIDs: [UInt8]? = nil) async throws -> Bool {
        if isBluetoothV3ProLightingDevice(device) {
            ...
            for ledID in bluetoothLightingLEDIDs(device: device, override: ledIDs) {
                let req = nextBTReq()
                let header = BLEVendorProtocol.buildWriteHeader(req: req, payloadLength: UInt8(payload.count), key: .lightingZoneStateSet(ledID: ledID))
                let notifies = try await btExchange([header, payload], timeout: 0.9, device: device)
        ...
        let payload = Data([0x04, 0x00, 0x00, 0x00, 0x00, UInt8(max(0, min(255, r))), UInt8(max(0, min(255, g))), UInt8(max(0, min(255, b)))])
        let req = nextBTReq()
    ...
    func btApplyLightingEffectFallback(device: MouseDevice, effect: LightingEffectPatch, ledIDs: [UInt8]? = nil) async throws -> Bool {
        switch effect.kind {
        case .off: return try await btSetLightingValue(device: device, value: 0)
        case .staticColor: return try await btSetLightingRGB(device: device, r: effect.primary.r, g: effect.primary.g, b: effect.primary.b, ledIDs: ledIDs)
        case .spectrum, .wave, .reactive, .pulseRandom, .pulseSingle, .pulseDual: return false
        }
    }
    ...
    func parseLightingRGB(payload: Data) -> RGBPatch? {
        ...
        if let zoneState = BLEVendorProtocol.parseV3ProLightingZoneStatePayload(payload) { return zoneState }
        if payload.count >= 8, payload[0] == 0x04 { return RGBPatch(r: Int(payload[5]), g: Int(payload[6]), b: Int(payload[7])) }
        if payload.count >= 4 { return RGBPatch(r: Int(payload[1]), g: Int(payload[2]), b: Int(payload[3])) }
        return nil
    }
