...
private struct KeyboardBindingRecorderPopover: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            KeyboardBindingSearchableKeyPicker(
                currentHidKey: currentHidKey,
                currentHidModifiers: currentHidModifiers,
                supportsModifierChords: supportsModifierChords
            ) { selection in
                onCapture(selection)
                dismiss()
...
private struct KeyboardBindingSearchableKeyPicker: View {
    ...
    private var isSelectingChordAction: Bool {
        pendingModifier != nil && supportsModifierChords
    }

    private var filteredOptions: [KeyboardBindingOption] {
        AppStateKeyboardSupport.filteredKeyOptions(
            matching: searchText,
            excludingModifiers: isSelectingChordAction
        )
    }
    ...
    private var searchPrompt: String {
        isSelectingChordAction ? "Search action key" : "Search keys"
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if let pendingModifier, supportsModifierChords {
                pendingModifierRow(pendingModifier)
            }
    ...
    private var searchField: some View {
        HStack(spacing: 8) {
            ...
            TextField(searchPrompt, text: $searchText)
                ...
                .accessibilityIdentifier("keyboard-binding-key-search")
    ...
    private var optionList: some View {
        ScrollView {
            LazyVStack(alignment: .leading, spacing: 10) {
                if groupedOptions.isEmpty {
                    ...
                    ForEach(groupedOptions, id: \.group.id) { entry in
                        optionSection(entry)
                    }
                }
            }
        ...
        .frame(width: pickerWidth, height: optionListHeight)
        .background(
            RoundedRectangle(cornerRadius: 8)
    ...
    private var emptySearchResult: some View {
        Text("No supported keys")
            .font(.system(size: 12, weight: .medium, design: .rounded))
            .foregroundStyle(.white.opacity(0.52))
            .frame(maxWidth: .infinity, minHeight: optionListHeight - 16)
    }
    ...
    private func optionButton(_ option: KeyboardBindingOption) -> some View {
        Button {
            ...
            HStack(spacing: 8) {
                Image(systemName: optionSystemImageName(for: option))
                    .font(.system(size: 12, weight: .bold))
                    .foregroundStyle(.white.opacity(0.48))
                    .frame(width: 18)
                    .accessibilityHidden(true)
            ...
            .background(
                RoundedRectangle(cornerRadius: 6)
                    .fill(Color.white.opacity(optionBackgroundOpacity(for: option)))
            )
        ...
        .onHover { isHovering in
            updateHoveredOption(option, isHovering: isHovering)
        }
        .accessibilityIdentifier("keyboard-binding-key-option-\(option.hidKey)")
    }

    @ViewBuilder
    private func optionTrailingIcon(_ option: KeyboardBindingOption) -> some View {
    ...
    private var pendingModifierBit: Int? {
        pendingModifier.flatMap { AppStateKeyboardSupport.hidModifierBit(forHidKey: $0.hidKey) }
    }

    private func optionIsSelected(_ option: KeyboardBindingOption) -> Bool {
        if isSelectingChordAction, let pendingModifierBit {
            return currentHidKey == option.hidKey && currentHidModifiers == pendingModifierBit
        }
    ...
    private func optionBackgroundOpacity(for option: KeyboardBindingOption) -> Double {
        if optionIsSelected(option) {
            return hoveredOptionHidKey == option.hidKey ? 0.14 : 0.10
        }
        return hoveredOptionHidKey == option.hidKey ? 0.07 : 0.0
    }

    private func updateHoveredOption(_ option: KeyboardBindingOption, isHovering: Bool) {
        if isHovering {
            hoveredOptionHidKey = option.hidKey
        } else if hoveredOptionHidKey == option.hidKey {
            hoveredOptionHidKey = nil
        }
    }

    private func optionSystemImageName(for option: KeyboardBindingOption) -> String {
        if AppStateKeyboardSupport.isModifierKey(option.hidKey) {
            return modifierSystemImageName(for: option.hidKey)
        }
        switch option.group {
        case .letters:
            return "textformat"
    ...
    private func modifierSystemImageName(for hidKey: Int) -> String {
        KeyboardBindingPickerIcons.modifierSystemImageName(for: hidKey)
    }
}
...
private struct KeyboardBindingPendingModifierRow: View {
    ...
    private var modifierIcon: some View {
        Image(systemName: KeyboardBindingPickerIcons.modifierSystemImageName(for: option.hidKey))
            .font(.system(size: 12, weight: .bold))
    ...
    private var useAloneButton: some View {
        Button("Use Alone", action: onUseAlone)
            .buttonStyle(.plain)
