...
enum BackgroundServiceTransportError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .connectionClosed: return "Background service connection closed unexpectedly"
        case .invalidLength: return "Background service returned an invalid message"
        case .missingPayload: return "Background service request was missing its payload"
        case .listenerUnavailable: return "Background service listener did not publish a port"
        }
    }
}
...
enum BackgroundServiceTransport {
    ...
    static func clientParameters() -> NWParameters { .tcp }
    ...
    static func sendFrame(_ payload: Data, over connection: NWConnection) async throws {
        ...
        withUnsafeBytes(of: &length) { header in framed.append(contentsOf: header) }
        framed.append(payload)

        try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in connection.send(content: framed, completion: .contentProcessed { error in if let error { continuation.resume(throwing: error) } else { continuation.resume() } }) }
    }
    ...
    static func receiveFrame(from connection: NWConnection) async throws -> Data {
        ...
        let length = header.reduce(UInt32(0)) { partial, byte in (partial << 8) | UInt32(byte) }
    ...
    private static func receiveExactly(_ count: Int, from connection: NWConnection) async throws -> Data {
        guard count >= 0 else { throw BackgroundServiceTransportError.invalidLength }
    ...
    private static func receiveChunk(maximumLength: Int, from connection: NWConnection) async throws -> Data {
        try await withCheckedThrowingContinuation { continuation in
            connection.receive(minimumIncompleteLength: 1, maximumLength: maximumLength) { data, _, isComplete, error in
                ...
                if isComplete { continuation.resume(throwing: BackgroundServiceTransportError.connectionClosed) } else { continuation.resume(throwing: BackgroundServiceTransportError.invalidLength) }
            }
        }
    }
}
