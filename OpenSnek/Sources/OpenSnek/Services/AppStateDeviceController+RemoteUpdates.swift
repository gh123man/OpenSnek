...
@MainActor extension AppStateDeviceController {
    func handleBackendDeviceListUpdate(_ listed: [MouseDevice], updatedAt: Date = Date()) async {
        ...
        let prioritizedDeviceIDs = listed.filter { $0.transport == .bluetooth && !previousIDs.contains($0.id) }.map(\.id)
    ...
    func applyRemoteServiceSnapshot(_ snapshot: SharedServiceSnapshot) {
        ...
        if applyRemoteStateCache(snapshot, liveIDs: liveIDs) { didApplySnapshotChange = true }
        ...
        if applySelectedRemoteSnapshotPresentation(deviceListChanged: deviceListChanged) { didApplySnapshotChange = true }

        if didApplySnapshotChange { Task { [weak self] in await self?.refreshDpiUpdateTransportStatuses(for: snapshot.devices) } }
    }
    ...
    func pruneRemoteSnapshotCaches(liveIDs: Set<String>, devices: [MouseDevice]) {
        ...
        for deviceID in Array(pendingUSBControlUnavailableTasksByDeviceID.keys) where !liveIDs.contains(deviceID) { cancelPendingUSBControlUnavailable(for: deviceID) }
        pruneUSBPhysicalConnectSettling(liveIDs: liveIDs)
    }

    func applyRemoteSoftwareLightingStatuses(_ snapshot: SharedServiceSnapshot, liveIDs: Set<String>) -> Bool {
        ...
        for (deviceID, previousStatus) in previousStatuses where liveIDs.contains(deviceID) {
            ...
            if let snapshotStatus = nextStatuses[deviceID], previousStatus.updatedAt <= snapshotStatus.updatedAt { continue }
    ...
    func applyRemoteUSBControlAvailability(_ snapshot: SharedServiceSnapshot, liveIDs: Set<String>) {
        for (deviceID, availability) in snapshot.usbControlAvailabilityByDeviceID where liveIDs.contains(deviceID) {
            ...
            if shouldDropStaleUSBControlUnavailable(availability, for: deviceID, observedAt: observedAt) {
                AppLog.debug("AppState", "remoteSnapshot usbControlAvailability stale-drop device=\(deviceID) " + "availability=\(availability.rawValue)")
                continue
            }
    ...
    func applyRemoteStateCache(_ snapshot: SharedServiceSnapshot, liveIDs: Set<String>) -> Bool {
        ...
        for (deviceID, remoteState) in snapshot.stateByDeviceID where liveIDs.contains(deviceID) {
            ...
            if shouldDropSupersededRemoteState(deviceID: deviceID, updatedAt: snapshotUpdatedAt) { continue }
            if stateCacheByDeviceID[deviceID] != remoteState || lastUpdatedByDeviceID[deviceID] != snapshotUpdatedAt {
    ...
    func applyRemoteUSBObservationIfNeeded(_ snapshot: SharedServiceSnapshot, deviceID: String, remoteState: MouseState, updatedAt snapshotUpdatedAt: Date) {
        guard remoteState.device.transport == .usb else { return }
        if snapshot.usbControlAvailabilityByDeviceID[deviceID] == nil { setUSBControlAvailability(.receiverPresentMouseReachable, for: deviceID, observedAt: snapshot.observedAtByDeviceID[deviceID] ?? Date()) }
        ...
        recordUSBLiveObservation(sourceDeviceID: deviceID, presentationDeviceID: deviceID, observedAt: observedAt, transportStatus: .pollingFallback)
    }

    func shouldDropSupersededRemoteState(deviceID: String, updatedAt snapshotUpdatedAt: Date) -> Bool {
        guard let latestCachedAt = lastUpdatedByDeviceID[deviceID], latestCachedAt > snapshotUpdatedAt else { return false }
        AppLog.debug("AppState", "remoteSnapshot superseded-drop device=\(deviceID) updatedAt=\(snapshotUpdatedAt.timeIntervalSince1970) " + "cachedAt=\(latestCachedAt.timeIntervalSince1970)")
        return true
    }

    func applySelectedRemoteSnapshotPresentation(deviceListChanged: Bool) -> Bool {
        if let selectedDeviceID = deviceStore.selectedDeviceID, let selectedState = stateCacheByDeviceID[selectedDeviceID], let selectedDevice = deviceStore.selectedDevice {
            return applySelectedRemoteState(selectedDeviceID: selectedDeviceID, selectedState: selectedState, selectedDevice: selectedDevice, deviceListChanged: deviceListChanged)
        }
        if let selectedDeviceID = deviceStore.selectedDeviceID { return applyMissingSelectedRemoteState(selectedDeviceID: selectedDeviceID, deviceListChanged: deviceListChanged) }
        return clearRemoteSelectionPresentation()
    }

    func applySelectedRemoteState(selectedDeviceID: String, selectedState: MouseState, selectedDevice: MouseDevice, deviceListChanged: Bool) -> Bool {
        var didApplySnapshotChange = false
        ...
        AppLog.debug("AppState", "remoteSnapshot selected device=\(selectedDeviceID) " + "active=\(activeStage) " + "dpi=\(dpi) " + "values=\(values) " + "scroll=\(scroll) " + "pendingLocal=\(pendingLocal) " + "pendingActive=\(pendingActive)")
        ...
        if selectedStateChanged || selectedLastUpdatedChanged || deviceListChanged { hydrateSelectedRemoteState(selectedState, selectedDevice: selectedDevice) }
        if deviceStore.errorMessage != nil {
            deviceStore.errorMessage = nil
            didApplySnapshotChange = true
        }
        didApplySnapshotChange = clearOrUpdateSelectedRemoteWarning(selectedState: selectedState, selectedDevice: selectedDevice) || didApplySnapshotChange
        return didApplySnapshotChange
    }

    func hydrateSelectedRemoteState(_ selectedState: MouseState, selectedDevice: MouseDevice) {
        let holdsPersistedConnectPresentation = primeSelectedConnectPresentationIfNeeded(device: selectedDevice, applyController: applyController, editorController: editorController)
        hydrateSelectedEditorPresentation(EditorPresentationHydrationRequest(state: selectedState, device: selectedDevice, holdsPersistedConnectPresentation: holdsPersistedConnectPresentation, applyController: applyController, editorController: editorController, scheduleButtonHydration: true))
    ...
    func applyMissingSelectedRemoteState(selectedDeviceID: String, deviceListChanged: Bool) -> Bool {
        if deviceListChanged { syncSelectedDevicePresentation(deviceID: selectedDeviceID) }
    ...
    func scheduleRemoteSnapshotSoftwareLightingAutoStart(for devices: [MouseDevice]) {
        ...
        for device in supportedDevices { remoteSnapshotSoftwareLightingAutoStartKeys.insert(DevicePersistenceKeys.key(for: device)) }
        Task { [weak self] in
            guard let self, !self.isTearingDown, let editorController = self.optionalEditorController else { return }
            for device in supportedDevices { await editorController.startPersistedSoftwareLightingOnConnectIfNeeded(for: device) }
        }
    }

    func applyBackendSoftwareLightingStatusUpdate(deviceID: String, status: SoftwareLightingEngineStatus?, updatedAt _: Date) { if let status { deviceStore.softwareLightingStatusByDeviceID[deviceID] = status } else { deviceStore.softwareLightingStatusByDeviceID.removeValue(forKey: deviceID) } }

    func applyBackendUSBControlAvailabilityUpdate(deviceID: String, availability: USBControlAvailability, updatedAt: Date) {
        guard !isTearingDown else { return }
        let previousSourceAvailability = usbControlAvailabilityByDeviceID[deviceID]
        let didApplySourceAvailability = setBackendObservedUSBControlAvailability(availability, for: deviceID, observedAt: updatedAt)

        guard let sourceDevice = deviceStore.devices.first(where: { $0.id == deviceID }), let presentationDevice = presentationDevice(for: sourceDevice) else { return }
    ...
    func applyBackendDeviceStateUpdate(deviceID: String, state updatedState: MouseState, updatedAt: Date) {
        ...
        let presentationDeviceID = presentationDevice.id
        if let latestCachedAt = latestCachedUpdateAt(sourceDeviceID: deviceID, presentationDeviceID: presentationDeviceID), latestCachedAt > updatedAt {
            AppLog.debug("AppState", "backendStateUpdate superseded-drop device=\(presentationDeviceID) updatedAt=\(updatedAt.timeIntervalSince1970) " + "cachedAt=\(latestCachedAt.timeIntervalSince1970)")
            return
        }
    ...
    func logBackendStateUpdate(sourceDeviceID: String, presentationDevice: MouseDevice, incoming updatedState: MouseState, merged: MouseState) {
        ...
        AppLog.debug(
            "AppState",
            "backendStateUpdate apply device=\(presentationDevice.id) source=\(sourceDeviceID) " + "incomingActive=\(incomingActive) " + "incomingDpi=\(incomingDpi) " + "incomingScroll=\(incomingScroll) " + "mergedActive=\(mergedActive) " + "mergedDpi=\(mergedDpi) " + "mergedValues=\(mergedValues) "
                + "mergedScroll=\(mergedScroll) " + "selected=\(selectedDeviceID) " + "pendingLocal=\(pendingLocal) " + "pendingActive=\(pendingActive)")
    }
    ...
    func applySelectedBackendStateUpdate(_ merged: MouseState, presentationDevice: MouseDevice) {
        ...
        hydrateSelectedEditorPresentation(EditorPresentationHydrationRequest(state: merged, device: presentationDevice, holdsPersistedConnectPresentation: holdsPersistedConnectPresentation, applyController: applyController, editorController: editorController, scheduleButtonHydration: true))
        deviceStore.errorMessage = nil
    ...
    func applyBackendDpiTransportStatusUpdate(deviceID: String, status: DpiUpdateTransportStatus, updatedAt: Date) {
        guard !isTearingDown else { return }
        if status == .streamActive { lastPassiveHeartbeatAtByDeviceID[deviceID] = updatedAt }

        let currentStatus = dpiUpdateTransportStatusByDeviceID[deviceID]
        let shouldApplySourceStatus = Self.shouldApplyBackendDpiTransportStatusUpdate(current: currentStatus, incoming: status)
        if !shouldApplySourceStatus, status != .streamActive { return }

        if shouldApplySourceStatus { setDpiUpdateTransportStatus(status, for: deviceID) }

        guard let sourceDevice = deviceStore.devices.first(where: { $0.id == deviceID }), let presentationDevice = presentationDevice(for: sourceDevice) else { return }

        let presentationDeviceID = presentationDevice.id
        if status == .streamActive { lastPassiveHeartbeatAtByDeviceID[presentationDeviceID] = updatedAt }
        let presentationStatus = dpiUpdateTransportStatusByDeviceID[presentationDeviceID]
        if Self.shouldApplyBackendDpiTransportStatusUpdate(current: presentationStatus, incoming: status) { setDpiUpdateTransportStatus(status, for: presentationDeviceID) }
        if sourceDevice.transport == .usb, status == .streamActive { recordUSBLiveObservation(sourceDeviceID: deviceID, presentationDeviceID: presentationDeviceID, observedAt: updatedAt, transportStatus: status) }
    }
