...
struct USBProbeDeviceCandidate: @unchecked Sendable {
    ...
    var usageLabel: String { String(format: "0x%02x:0x%02x", usagePage, usage) }

    func describe() -> String { String(format: "candidate[%d] %@ pid=0x%04x loc=0x%08x usage=%@ input=%d feature=%d score=%d name=%@", index, deviceID, productID, locationID, usageLabel, maxInputReportSize, maxFeatureReportSize, score, productName) }
}
...
struct BTHIDProbeDeviceCandidate: @unchecked Sendable {
    ...
    func describe() -> String {
        let format = "candidate[%d] %@ vid=0x%04x pid=0x%04x loc=0x%08x usage=%@ input=%d feature=%d " + "score=%d transport=\"%@\" name=%@"
        return String(format: format, index, deviceID, vendorID, productID, locationID, usageLabel, maxInputReportSize, maxFeatureReportSize, score, transport, productName)
    }
}
...
func enumerateUSBProbeCandidates(preferredProductID: Int? = nil) throws -> (manager: IOHIDManager, candidates: [USBProbeDeviceCandidate]) {
    let usbVID = 0x1532
    ...
    for candidate in rawSet {
        guard USBHIDSupport.intProperty(candidate, key: kIOHIDVendorIDKey as CFString) == usbVID, let product = USBHIDSupport.intProperty(candidate, key: kIOHIDProductIDKey as CFString) else { continue }
        ...
        gathered.append(
            USBProbeCandidateSeed(
                device: candidate, deviceID: deviceID, productID: product, productName: productName, locationID: locationID, usagePage: usagePage, usage: usage, maxInputReportSize: maxInputReportSize, maxFeatureReportSize: maxFeatureReportSize, score: score, passiveDescriptor: passiveDescriptor))
    }

    guard !gathered.isEmpty else {
        if let preferredProductID { throw ProbeError.protocolError("No non-Bluetooth USB Razer HID interface found for pid 0x\(String(format: "%04x", preferredProductID))") }
    ...
    let candidates = sorted.enumerated().map { index, candidate in
        USBProbeDeviceCandidate(
            index: index, device: candidate.device, devicePointer: UInt(bitPattern: Unmanaged.passUnretained(candidate.device).toOpaque()), deviceID: candidate.deviceID, productID: candidate.productID, productName: candidate.productName, locationID: candidate.locationID,
            usagePage: candidate.usagePage, usage: candidate.usage, maxInputReportSize: candidate.maxInputReportSize, maxFeatureReportSize: candidate.maxFeatureReportSize, score: candidate.score, passiveDescriptor: candidate.passiveDescriptor)
    }
    return (manager, candidates)
}

func enumerateBTHIDProfileCandidates(preferredProductID: Int? = 0x00AC, preferredPeripheralName: String? = nil) throws -> (manager: IOHIDManager, candidates: [BTHIDProbeDeviceCandidate]) {
    ...
    for candidate in rawSet {
        guard let vendorID = USBHIDSupport.intProperty(candidate, key: kIOHIDVendorIDKey as CFString), supportedVendorIDs.contains(vendorID), let productID = USBHIDSupport.intProperty(candidate, key: kIOHIDProductIDKey as CFString) else { continue }
        ...
        guard transportLower.contains("bluetooth") || transportLower.contains("ble") || vendorID == 0x068E else { continue }

        let productName = USBHIDSupport.stringProperty(candidate, key: kIOHIDProductKey as CFString) ?? "Razer Bluetooth HID Device"
        if let preferredPeripheralName, !BluetoothNameMatcher.looselyMatches(productName, preferredPeripheralName) { continue }
        ...
        if let descriptor {
            descriptorScore = (usagePage == descriptor.usagePage ? 20 : 0) + (usage == descriptor.usage ? 20 : 0) + (maxInputReportSize >= descriptor.minInputReportSize ? 10 : 0) + (descriptor.maxFeatureReportSize == nil || maxFeatureReportSize == descriptor.maxFeatureReportSize ? 5 : 0)
        } else {
            descriptorScore = 0
        }
        ...
        gathered.append(
            BTHIDProbeCandidateSeed(
                device: candidate, deviceID: deviceID, vendorID: vendorID, productID: productID, productName: productName, transport: transport, locationID: locationID, usagePage: usagePage, usage: usage, maxInputReportSize: maxInputReportSize, maxFeatureReportSize: maxFeatureReportSize,
                score: score, passiveDescriptor: descriptor))
    }
    ...
    let candidates = sorted.enumerated().map { index, candidate in
        BTHIDProbeDeviceCandidate(
            index: index, device: candidate.device, deviceID: candidate.deviceID, vendorID: candidate.vendorID, productID: candidate.productID, productName: candidate.productName, transport: candidate.transport, locationID: candidate.locationID, usagePage: candidate.usagePage,
            usage: candidate.usage, maxInputReportSize: candidate.maxInputReportSize, maxFeatureReportSize: candidate.maxFeatureReportSize, score: candidate.score, passiveDescriptor: candidate.passiveDescriptor)
    }
    return (manager, candidates)
}
