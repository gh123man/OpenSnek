...
extension OpenSnekProbe {
  static func runBluetoothCommand(_ command: String, commandArgs: [String]) async throws {
    ...
    case "bt-raw-read":
      ...
      print(
        "bt-raw-read req=0x\(String(format: "%02x", result.req)) key=\(hexString(parsed.key)) name=\"\(parsed.preferredPeripheralName ?? "")\""
      )
    ...
    case "bt-raw-write":
      ...
      print(
        "bt-raw-write req=0x\(String(format: "%02x", result.req)) key=\(hexString(parsed.key)) payload=\(hexString(parsed.payload)) name=\"\(parsed.preferredPeripheralName ?? "")\""
      )
      print(describeBTNotifyFrames(result.notifies))
      if let ack = result.ack {
        print(
          "ack status=0x\(String(format: "%02x", ack.status)) payloadLength=\(ack.payloadLength)")
      } else {
        print("ack: nil")
      }
    case "bt-profile-read":
      ...
      try await printBTProfileReadSweep(
        BTProfileReadSweepRequest(
          session: BTProfileProbeSession(
            bridge: bridge,
            preferredPeripheralName: parsed.preferredPeripheralName,
    ...
    case "bt-profile-active-set":
      ...
      guard write.ack?.status == 0x02, afterTarget == parsed.target else {
        throw ProbeError.protocolError(
          "BT active-target selector did not select target \(parsed.target)")
      }
    ...
    case "bt-profile-button-read":
      ...
      let key = BLEVendorProtocol.Key.buttonBindGet(target: parsed.target, slot: parsed.buttonSlot)
        .bytes
      let result = try await bridge.rawRead(
        key: key,
        timeout: parsed.timeoutSeconds,
    ...
    case "bt-profile-button-set":
      ...
      let storedKey = BLEVendorProtocol.Key.buttonBindSet(
        target: parsed.target, slot: parsed.buttonSlot
      ).bytes
      ...
      if parsed.projectLive {
        let livePayload = Array(
          BLEVendorProtocol.retargetButtonPayload(
            Data(storedPayload),
            target: 0x01,
            slot: parsed.buttonSlot
          ))
        let liveKey = BLEVendorProtocol.Key.buttonBindSet(target: 0x01, slot: parsed.buttonSlot)
          .bytes
      ...
      for target in readbackTargets {
        let readKey = BLEVendorProtocol.Key.buttonBindGet(target: target, slot: parsed.buttonSlot)
          .bytes
        ...
        print(
          describeBTProfileButtonRead(
            key: readKey, payload: readback.payload, notifies: readback.notifies))
      }
    ...
    case "bt-profile-watch":
      ...
      print(
        "note: this fingerprints the live projected Bluetooth layer, not the persistent stored slots."
      )
    ...
    case "bt-lighting-info":
      ...
      let profile = await bridge.bluetoothLightingProfile(
        preferredPeripheralName: parsed.preferredPeripheralName)
      let zoneChoices = await bridge.bluetoothLightingZoneChoices(
        preferredPeripheralName: parsed.preferredPeripheralName)
      let supportedLEDIDs = try await bridge.bluetoothLightingLEDIDs(
        preferredPeripheralName: parsed.preferredPeripheralName)
      print(
        "bt profile=\"\(profile?.productName ?? "unknown")\" name=\"\(parsed.preferredPeripheralName ?? "")\""
      )
      ...
      if let targets = try await bridge.bluetoothLightingTargets(
        preferredPeripheralName: parsed.preferredPeripheralName) {
      ...
      guard
        let reads = try await bridge.readBluetoothLighting(
          preferredPeripheralName: parsed.preferredPeripheralName,
    ...
    case "bt-lighting-brightness":
      ...
      let zoneChoices = await bridge.bluetoothLightingZoneChoices(
        preferredPeripheralName: parsed.preferredPeripheralName)
      guard
        let writes = try await bridge.writeBluetoothLightingBrightness(
          value: parsed.value,
          preferredPeripheralName: parsed.preferredPeripheralName,
          zoneID: parsed.zoneID
        )
    ...
    case "bt-lighting-color":
      ...
      let zoneChoices = await bridge.bluetoothLightingZoneChoices(
        preferredPeripheralName: parsed.preferredPeripheralName)
      guard
        let writes = try await bridge.writeBluetoothLightingColor(
          color: parsed.color,
          preferredPeripheralName: parsed.preferredPeripheralName,
          zoneID: parsed.zoneID
        )
  ...
  static func runUSBCommand(_ command: String, commandArgs: [String]) async throws {
    ...
    case "usb-profile-active-set":
      ...
      print(
        "usb-profile-active-set \(usb.describe()) profile=\(parsed.profile) \(usbProfileLabel(parsed.profile))"
      )
      let selected = try usb.writeActiveProfileID(parsed.profile)
      let after = try usb.readActiveProfileID()
      print(
        "active-profile-set command=05:04 " + "accepted=\(selected ? "yes" : "no") "
          + "before=\(before.map(String.init) ?? "nil") "
          + "after=\(after.map(String.init) ?? "nil")"
      )
      guard selected else {
        throw ProbeError.protocolError(
          "USB profile \(parsed.profile) was rejected by active-profile selector 05:04")
      }
    ...
    case "usb-profile-verify-changed-writes":
      let parsed = try parseUSBProfileVerifyWritesArgs(
        commandArgs, command: "usb-profile-verify-changed-writes")
      let usb = try USBProbeClient(productID: parsed.productID)
    ...
    case "usb-profile-verify-metadata-write":
      throw ProbeError.protocolError(
        "usb-profile-verify-metadata-write is disabled: 05:08 metadata chunks are mapped, but create/assign can disturb profile content and needs a guarded rewrite/readback probe"
      )
    case "usb-profile-delete":
      ...
      print(
        "delete-unassign \(usbProfileLabel(parsed.profile)) command=05:03 status=\(deleted ? "ok" : "read_failed")"
      )
      guard deleted else {
        throw ProbeError.protocolError(
          "USB profile delete/unassign did not echo the expected command")
      }
      ...
      if let battery = try usb.readBattery() {
        print(
          "battery charging=\(battery.charging ? "yes" : "no") "
            + "raw=0x\(String(format: "%02x", battery.rawLevel)) " + "percent=\(battery.percent)"
        )
      } else {
    ...
    case "usb-lighting-info":
      ...
      print("usb \(usb.describe())")
      print(
        "supported-effects=\(usb.supportedLightingEffects().map(\.rawValue).joined(separator: ","))"
      )
    ...
    case "usb-lighting-brightness":
      ...
      let usb = try USBProbeClient(productID: parsed.productID)
      print("usb \(usb.describe())")
      guard let writes = try usb.writeLightingBrightness(value: parsed.value, zoneID: parsed.zoneID)
      else {
        throw invalidUSBLightingZone(zoneID: parsed.zoneID, usb: usb)
      }
      for write in writes {
    ...
    case "usb-lighting-effect":
      ...
      guard let writes = try usb.writeLightingEffect(effect: parsed.effect, zoneID: parsed.zoneID)
      else {
        throw invalidUSBLightingZone(zoneID: parsed.zoneID, usb: usb)
      }
      for write in writes {
    ...
    case "usb-input-listen":
      ...
      print(
        "usb-input-listen candidates=\(probe.candidateCount) duration=\(String(format: "%.1f", parsed.durationSeconds))s"
      )
      for line in probe.describeCandidates() {
        print(line)
      }
    ...
    case "usb-input-values":
      ...
      print(
        "usb-input-values candidates=\(probe.candidateCount) duration=\(String(format: "%.1f", parsed.durationSeconds))s"
      )
      for line in probe.describeCandidates() {
        print(line)
      }
    ...
    case "usb-button-read":
      ...
      for profile in parsed.profiles {
        if let block = try usb.readButtonFunction(
          profile: profile, slot: slot, hypershift: parsed.hypershift) {
          print(
            "profile=\(profile) slot=\(parsed.slot) hypershift=\(parsed.hypershift) \(describeUSBFunctionBlock(block))"
          )
        } else {
          print(
            "profile=\(profile) slot=\(parsed.slot) hypershift=\(parsed.hypershift) read_failed")
        }
      }
    case "usb-button-set":
      ...
      for profile in parsed.profiles {
        if let block = try usb.readButtonFunction(profile: profile, slot: slot, hypershift: 0x00) {
          print(
            "readback profile=\(profile) slot=\(parsed.slot) \(describeUSBFunctionBlock(block))")
        }
      }
    case "usb-button-set-raw":
      ...
      var wroteAny = false
      for profile in parsed.profiles
      where try usb.writeButtonFunction(
        profile: profile,
        slot: slot,
        hypershift: 0x00,
        functionBlock: parsed.functionBlock
      ) {
        wroteAny = true
      }
      guard wroteAny else {
