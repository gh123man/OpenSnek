...
enum BTProfileHIDClassification: Equatable, Sendable {
    ...
    var label: String {
        switch self {
        case .profileCyclePrelude: return "profile-cycle-prelude"
        case .profileCycleFollowUp: return "profile-cycle-followup"
        case .dpi(let reading): return "dpi=\(reading.dpiX)x\(reading.dpiY)"
    ...
    var isProfileCycleHint: Bool {
        switch self {
        case .profileCyclePrelude, .profileCycleFollowUp: return true
        case .dpi, .heartbeat, .other: return false
        }
    }
}
...
final class BTProfileHIDReportProbe: @unchecked Sendable {
    ...
    private final class CallbackContext {
        ...
        init(candidate: BTHIDProbeDeviceCandidate, captureStartedAt: Date, emit: @escaping @Sendable (BTProfileHIDReportEvent) -> Void) {
    ...
    init(productID preferredProductID: Int? = 0x00AC, preferredPeripheralName: String? = nil) throws {
        let enumeration = try enumerateBTHIDProfileCandidates(preferredProductID: preferredProductID, preferredPeripheralName: preferredPeripheralName)
    ...
    func describeCandidates() -> [String] { candidates.map { $0.describe() } }

    func capture(duration: TimeInterval, maxReports: Int? = nil, onReport: @escaping @Sendable (BTProfileHIDReportEvent) -> Void) async throws -> Int {
        try await start(onReport: onReport)
        defer { stopSynchronously() }

        let deadline = Date().addingTimeInterval(max(0.1, duration))
        while Date() < deadline {
            if let maxReports, currentReportCount() >= maxReports { break }
        ...
        try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
            queue.async {
                ...
                self.performOnRunLoopLocked {
                    ...
                    for candidate in self.candidates { _ = self.addRegistration(candidate: candidate, onReport: onReport) }

                    if self.registrationsByIndex.isEmpty { continuation.resume(throwing: ProbeError.protocolError("Failed to register any Bluetooth HID input-report callbacks")) } else { continuation.resume() }
                }
            }
        }
    }
    ...
    private func addRegistration(candidate: BTHIDProbeDeviceCandidate, onReport: @escaping @Sendable (BTProfileHIDReportEvent) -> Void) -> Bool {
        ...
        let contextBox = CallbackContext(
            candidate: candidate, captureStartedAt: captureStartedAt,
        ...
        registrationsByIndex[candidate.index] = Registration(device: registrationDevice, buffer: buffer, bufferLength: CFIndex(reportLength), context: context)
        return true
    }

    private func removeAllRegistrations() { for index in Array(registrationsByIndex.keys) { removeRegistration(index: index) } }
    ...
    private static let inputReportCallback: IOHIDReportCallback = { context, result, _, type, _, report, length in
        guard result == kIOReturnSuccess, type == kIOHIDReportTypeInput, let context else { return }
        ...
        callbackContext.emit(
            BTProfileHIDReportEvent(
                candidateIndex: candidate.index, deviceID: candidate.deviceID, productID: candidate.productID, productName: candidate.productName, usagePage: candidate.usagePage, usage: candidate.usage, maxInputReportSize: candidate.maxInputReportSize,
                maxFeatureReportSize: candidate.maxFeatureReportSize, report: bytes, elapsedSeconds: Date().timeIntervalSince(callbackContext.captureStartedAt), classification: classify(report: bytes, descriptor: candidate.passiveDescriptor)))
    }

    private static func classify(report: [UInt8], descriptor: PassiveDPIInputDescriptor?) -> BTProfileHIDClassification {
        if isProfileCyclePrelude(report) { return .profileCyclePrelude }
        if payloadStartIndex(in: report, reportID: 0x05, allowedSubtypes: [0x39]) != nil { return .profileCycleFollowUp }
        if let descriptor {
            switch PassiveDPIParser.classify(report: report, descriptor: descriptor) {
            case .dpi(let reading): return .dpi(reading)
            case .heartbeat: return .heartbeat
            case .profileSwitch: return .profileCycleFollowUp
            case .other: break
            }
        }
        return .other
    ...
    private static func payloadStartIndex(in report: [UInt8], reportID: UInt8, allowedSubtypes: [UInt8]) -> Int? {
