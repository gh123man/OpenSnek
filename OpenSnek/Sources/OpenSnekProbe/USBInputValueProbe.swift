...
struct USBInputValueEvent: Sendable {
    ...
    var deviceUsageLabel: String { String(format: "0x%02x:0x%02x", deviceUsagePage, deviceUsage) }

    var elementUsageLabel: String { String(format: "0x%04x:0x%04x", elementUsagePage, elementUsage) }
}
...
final class USBInputValueProbe: @unchecked Sendable {
    ...
    private final class CallbackContext {
        ...
        init(emit: @escaping @Sendable (IOHIDValue, UInt?) -> Void) { self.emit = emit }
    }
    ...
    func capture(duration: TimeInterval, maxEvents: Int? = nil, onValue: @escaping @Sendable (USBInputValueEvent) -> Void) async throws -> Int {
        try await start(onValue: onValue)
        defer { stopSynchronously() }

        let deadline = Date().addingTimeInterval(max(0.1, duration))
        while Date() < deadline {
            if let maxEvents, currentEventCount() >= maxEvents { break }
            ...
            queue.async {
                ...
                self.performOnRunLoopLocked {
                    ...
                    let contextBox = CallbackContext { [weak self] value, senderPointer in
                        guard let self, let senderPointer, let candidate = candidateByPointer[senderPointer] else { return }
                        let element = IOHIDValueGetElement(value)
                        let observedAt = Date()
                        self.incrementEventCount()
                        onValue(
                            USBInputValueEvent(
                                candidateIndex: candidate.index, deviceUsagePage: candidate.usagePage, deviceUsage: candidate.usage, elementUsagePage: Int(IOHIDElementGetUsagePage(element)), elementUsage: Int(IOHIDElementGetUsage(element)), reportID: Int(IOHIDElementGetReportID(element)),
                                integerValue: IOHIDValueGetIntegerValue(value), elapsedSeconds: observedAt.timeIntervalSince(captureStartedAt)))
                    }
