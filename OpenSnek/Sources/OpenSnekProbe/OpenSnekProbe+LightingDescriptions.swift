...
extension OpenSnekProbe {
    ...
    static func describeUSBLightingWriteResult(_ result: USBLightingWriteResult, operation: String) -> String {
    ...
    static func describeUSBLightingConcurrencyResult(_ result: USBLightingConcurrencyProbeResult) -> String {
        ["mode=\(result.mode)", String(format: "elapsed=%.1fms", result.elapsedMs), describeUSBLightingConcurrencyStats("frames", result.frameStats), describeUSBLightingConcurrencyStats("reads", result.commandReadStats), describeUSBLightingConcurrencyStats("writes", result.commandWriteStats)]
            .joined(separator: " ")
    }

    static func describeUSBLightingConcurrencyStats(_ label: String, _ stats: USBLightingConcurrencyOperationStats) -> String { String(format: "%@=%d/%d fail=%d avg=%.2fms max=%.2fms", label, stats.successes, stats.attempts, stats.failures, stats.averageMs, stats.maxMs) }

    static func hexLEDIDList(_ ledIDs: [UInt8]) -> String { ledIDs.map { String(format: "0x%02x", $0) }.joined(separator: ",") }

    static func describeBTLightingTarget(_ target: USBLightingTargetDescriptor) -> String { "zone id=\(target.zoneID) label=\"\(target.zoneLabel)\" ledIDs=[0x\(String(format: "%02x", target.ledID))]" }

    static func describeBTLightingReadResult(_ result: BTLightingReadResult) -> String {
        let brightness = result.brightness.map(String.init) ?? "read_failed"
        let color = result.color.map { color in String(format: "%02x%02x%02x", color.r, color.g, color.b) } ?? "read_failed"
    ...
    static func describeBTLightingWriteResult(_ result: BTLightingWriteResult, operation: String) -> String {
