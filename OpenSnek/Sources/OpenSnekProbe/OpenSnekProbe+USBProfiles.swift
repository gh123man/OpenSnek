...
extension OpenSnekProbe {
  ...
  static func printUSBProfileReadSweep(
    usb: USBProbeClient,
    profiles: [UInt8],
    buttonSlots: [UInt8],
    ...
    if let summary = try usb.readProfileSummaryRaw() {
      ...
      print(
        "summary class=00 cmd=87 payload=\(hexString(summary)) activeHint=\(activeHint) countHint=\(countHint)"
      )
    } else {
    ...
    if let inventory = try usb.readProfileInventory() {
      let profiles = inventory.assignedProfiles
        ...
        .joined(separator: ",")
      print(
        "profile-inventory class=05 cmd=81 maxProfile=0x\(String(format: "%02x", inventory.maxProfileID)) assigned=[\(profiles)]"
      )
    } else {
    ...
    print(
      "lighting-leds ids=\(lightingTargets.map { String(format: "0x%02x", $0.ledID) }.joined(separator: ","))"
    )
    ...
    for profile in readProfiles {
      ...
      if profile >= 0x02 {
        if let metadataRead = try usb.readProfileMetadata(profile: profile) {
          ...
          print(
            "metadata \(label) chunks=\(metadataRead.chunks.count) offsets=[\(chunkOffsets)] uuid=\(uuid) name=\(name) owner=\(owner)"
          )
        } else {
      ...
      for target in lightingTargets {
        if let brightness = try usb.readProfileLightingBrightness(
          profile: profile, ledID: target.ledID) {
          ...
          print(
            "brightness \(label) zone=\(target.zoneID) led=0x\(String(format: "%02x", target.ledID)) read_failed"
          )
        }
      }
      ...
      for slot in buttonSlots {
        if let block = try usb.readButtonFunction(profile: profile, slot: slot) {
          ...
          print(
            "button \(label) slot=\(slot) raw=\(hexString(block)) \(describeUSBFunctionBlock(block))"
          )
        } else {
  ...
  static func verifyUSBProfileSameValueWrites(usb: USBProbeClient, profile: UInt8) throws {
    ...
    let stagesMatch =
      stagesAfter.activeToken == stagesBefore.activeToken
      && stagesAfter.stageIDs == stagesBefore.stageIDs && stagesAfter.pairs == stagesBefore.pairs
    print(
    ...
    for target in usb.profileLightingTargets() {
      guard
        let brightnessBefore = try usb.readProfileLightingBrightness(
          profile: profile, ledID: target.ledID),
        let valueBefore = brightnessBefore.brightness
      else {
        throw ProbeError.protocolError(
          "Unable to read brightness before same-value write for LED 0x\(String(format: "%02x", target.ledID))"
        )
      }
      let brightnessWrite = try usb.writeProfileLightingBrightness(
        profile: profile, ledID: target.ledID, brightness: valueBefore)
      guard
        let brightnessAfter = try usb.readProfileLightingBrightness(
          profile: profile, ledID: target.ledID),
        let valueAfter = brightnessAfter.brightness
      else {
        throw ProbeError.protocolError(
          "Unable to read brightness after same-value write for LED 0x\(String(format: "%02x", target.ledID))"
        )
      }
      print(
      ...
      guard brightnessWrite, valueAfter == valueBefore else {
        throw ProbeError.protocolError(
          "Brightness same-value write did not round-trip for LED 0x\(String(format: "%02x", target.ledID))"
        )
      }
    }
  }
  ...
  static func verifyUSBProfileChangedValueWrites(usb: USBProbeClient, profile: UInt8) throws {
    ...
    do {
      ...
      for record in original.brightness {
        ...
        guard
          let brightnessAfter = try usb.readProfileLightingBrightness(
            profile: profile, ledID: record.target.ledID),
          let valueAfter = brightnessAfter.brightness
        else {
          throw ProbeError.protocolError(
            "Unable to read brightness after changed-value write for LED 0x\(String(format: "%02x", record.target.ledID))"
          )
        }
        print(
        ...
        guard brightnessWrite, valueAfter == changedValue else {
          throw ProbeError.protocolError(
            "Brightness changed-value write did not round-trip for LED 0x\(String(format: "%02x", record.target.ledID))"
          )
        }
      }
  ...
  static func cloneUSBProfile(_ request: USBProfileCloneRequest) throws {
    ...
    guard let sourceIdentifier = sourceMetadata.identifier,
      ...
      let compatibleSourceOwner = OnboardProfileMetadata.synapseCompatibleOwner(
        from: sourceMetadata.owner)
    else {
    ...
    case .repair:
      ...
      metadataExpectation =
        "uuid=\(repairedIdentifier.uuidString.lowercased()) name=\"\(repairedName)\" owner=\(compatibleSourceOwner)"
    }

    print(
      "clone metadata-plan \(targetLabel) mode=\(request.metadataMode.rawValue) expected=\(metadataExpectation)"
    )
    ...
    let expectedMetadata = Array(
      metadataToWrite.prefix(USBHIDProtocol.onboardProfileMetadataLength))
    let actualMetadata = Array(
      targetMetadataRead.bytes.prefix(USBHIDProtocol.onboardProfileMetadataLength))
    ...
    guard let sourceScalar = try usb.readProfileDPIScalar(profile: sourceProfile),
      let sourcePair = sourceScalar.pair
    else {
    ...
    guard let targetScalar = try usb.readProfileDPIScalar(profile: targetProfile),
      let targetPair = targetScalar.pair
    else {
    ...
    for target in usb.profileLightingTargets() {
      guard
        let sourceBrightness = try usb.readProfileLightingBrightness(
          profile: sourceProfile, ledID: target.ledID),
        let sourceValue = sourceBrightness.brightness
      else {
        throw ProbeError.protocolError(
          "Unable to read source brightness for LED 0x\(String(format: "%02x", target.ledID))")
      }
      let brightnessWrite = try usb.writeProfileLightingBrightness(
        profile: targetProfile,
        ledID: target.ledID,
      ...
      guard
        let targetBrightness = try usb.readProfileLightingBrightness(
          profile: targetProfile, ledID: target.ledID),
        let targetValue = targetBrightness.brightness
      else {
        throw ProbeError.protocolError(
          "Unable to read target brightness for LED 0x\(String(format: "%02x", target.ledID)) after clone"
        )
      }
      print(
      ...
      guard brightnessWrite, targetValue == sourceValue else {
        throw ProbeError.protocolError(
          "Target brightness did not match source for LED 0x\(String(format: "%02x", target.ledID))"
        )
      }
    }
    ...
    for slot in request.buttonSlots {
      ...
      let buttonWrite = try usb.writeButtonFunction(
        profile: targetProfile, slot: slot, functionBlock: sourceBlock)
      ...
      guard buttonWrite, targetBlock == sourceBlock else {
        throw ProbeError.protocolError(
          "Target button slot \(slot) did not match source after clone")
      }
    }
  }
  ...
  static func repairedTargetName(current: String?, source: String, targetProfile: UInt8)
    -> String {
  ...
  static func repairedUSBProfileMetadataBytes(
    base: [UInt8],
    identifier: UUID,
    name: String,
    owner: String
    ...
    if metadata.count < USBHIDProtocol.onboardProfileMetadataLength {
      metadata.append(
        contentsOf: repeatElement(
          0x00, count: USBHIDProtocol.onboardProfileMetadataLength - metadata.count))
    }
    ...
    try writeASCIIField(
      name, into: &metadata, offset: 0x10, maxLength: 0x74 - 0x10, fieldName: "profile name")
    try writeASCIIField(
      owner, into: &metadata, offset: 0x74, maxLength: 64, fieldName: "profile owner")
    return metadata
  }

  static func writeASCIIField(
    _ value: String,
  ...
  private static func readUSBProfileWritableSnapshot(usb: USBProbeClient, profile: UInt8) throws
    -> USBProfileWritableSnapshot {
    ...
    for target in usb.profileLightingTargets() {
      guard let read = try usb.readProfileLightingBrightness(profile: profile, ledID: target.ledID),
        ...
        throw ProbeError.protocolError(
          "Unable to read brightness snapshot for LED 0x\(String(format: "%02x", target.ledID))")
      }
  ...
  private static func restoreUSBProfileWritableSnapshot(
    ...
    let scalarWrite = try usb.writeProfileDPIScalar(
      profile: snapshot.profile, pair: snapshot.scalar)
    ...
    for record in snapshot.brightness {
      ...
      guard
        let brightnessAfter = try usb.readProfileLightingBrightness(
          profile: snapshot.profile, ledID: record.target.ledID),
        let valueAfter = brightnessAfter.brightness
      else {
        throw ProbeError.protocolError(
          "Unable to read brightness after restore for LED 0x\(String(format: "%02x", record.target.ledID))"
        )
      }
      print(
      ...
      guard brightnessWrite, valueAfter == record.value else {
        throw ProbeError.protocolError(
          "Brightness restore did not round-trip for LED 0x\(String(format: "%02x", record.target.ledID))"
        )
      }
    }
  }
  ...
  static func changedUSBProfileBrightnessValue(_ value: Int) -> Int {
