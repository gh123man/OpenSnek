...
final class USBInputReportProbe: @unchecked Sendable {
    ...
    private final class CallbackContext {
        ...
        init(emit: @escaping @Sendable ([UInt8]) -> Void) { self.emit = emit }
    }
    ...
    func capture(duration: TimeInterval, maxReports: Int? = nil, onReport: @escaping @Sendable (USBInputReportEvent) -> Void) async throws -> Int {
        ...
        try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
            queue.async {
                ...
                self.performOnRunLoopLocked {
                    ...
                    if self.registrationsByIndex.isEmpty { continuation.resume(throwing: ProbeError.protocolError("Failed to register any USB input-report callbacks")) } else { continuation.resume() }
                }
            }
        }
    }
    ...
    private func addRegistration(candidate: USBProbeDeviceCandidate, onReport: @escaping @Sendable (USBInputReportEvent) -> Void) -> Bool {
        ...
        let contextBox = CallbackContext { [weak self] report in
            ...
            let observedAt = Date()
            let passiveDPI = candidate.passiveDescriptor.flatMap { PassiveDPIParser.parse(report: report, descriptor: $0) }
            self.incrementReportCount()
            onReport(
                USBInputReportEvent(
                    candidateIndex: candidate.index, usagePage: candidate.usagePage, usage: candidate.usage, maxInputReportSize: candidate.maxInputReportSize, maxFeatureReportSize: candidate.maxFeatureReportSize, report: report, elapsedSeconds: observedAt.timeIntervalSince(captureStartedAt),
        ...
        IOHIDDeviceRegisterInputReportCallback(candidate.device, buffer, CFIndex(reportLength), Self.inputReportCallback, context)

        registrationsByIndex[candidate.index] = Registration(device: candidate.device, buffer: buffer, bufferLength: CFIndex(reportLength), context: context)
        return true
    }
