...
- Fixed Basilisk V3 Pro USB active-profile hydration to parse the validated `05:84` response shape even when the response size byte is `0x00`, and to prefer that direct active-profile register over the stale `00:87` active hint. Follow-up bulk probes mapped `05:81` as assigned-profile inventory and `05:80` as its count hint.
