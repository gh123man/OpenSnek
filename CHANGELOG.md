...
### Fixed
- The macOS button-remap UI now uses a single library-first `Buttons` card. It presents one live working copy with a `Current Profile` picker, explicit `Apply` / `Store...` actions, saved OpenSnek profiles alongside mouse slots, quieter live/default status text, and a lightweight profile-management sheet instead of the older slot-chip grid.
