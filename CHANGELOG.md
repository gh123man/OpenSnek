...
- Added a Synapse-closed Bluetooth physical profile-button capture. The draft spec now records that no BLE vendor profile-cycle traffic was observed with Synapse closed, while follow-up observation confirms both Bluetooth and USB support firmware/onboard profile-button cycling when Synapse is not taking over the device. OpenSnek's target model is firmware-first: trust onboard profile cycling by default and treat Synapse's software interception as a vendor UI behavior.
