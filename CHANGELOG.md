...
- The profile picker now hides the local profile already loaded in the selected slot, labels empty onboard slots as Load Profile, uses device defaults for Start Fresh, auto-loads newly created profiles into the selected slot, avoids duplicate fallback `Profile 1` local backups from metadata-less reads, and distinguishes deleting a local profile from deleting an onboard slot assignment.
