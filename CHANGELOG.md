...
- Added a Synapse-closed Bluetooth physical profile-button capture. The draft spec now records that no BLE vendor profile-cycle traffic was observed with Synapse closed and treats Bluetooth profile cycling as host-orchestrated software behavior. It also notes the observed USB distinction: USB can hardware-cycle onboard profiles without Synapse, but Synapse appears to take over the USB profile button as software-only navigation while connected.
