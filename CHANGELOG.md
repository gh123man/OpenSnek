...
- Cross-checked OpenRazer's unknown-command onboard-memory notes against the attached V3 Pro and mapped USB metadata reads through `05:88`. Slot `2` and `3` metadata chunks returned the same UUID/name data read over Bluetooth, including `OPENSNEK_MAC_SLOT_1` and `OPENSNEK_RECREATE_SLOT_2`; the likely `05:08` metadata write counterpart remains guarded research-only.
...
- Added `OpenSnekProbe usb-profile-verify-metadata-write`, a guarded UUID/name write research helper. A simple `05:08` offset-`0` write attempt against profile `5` did not change `05:88` readback, and profile `5` remained intact, so USB metadata writes remain unmapped.
