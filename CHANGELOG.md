...
- Added a Windows event-driven Basilisk V3 Pro Bluetooth profile-cycle watcher. The follow-up read capture confirms passive HID hints can drive reactive UI refreshes without polling, but known live-target readback keys (`0B 84 01 00`, `08 84 01 04`) stayed unchanged after Synapse-closed hardware profile cycles, so exact firmware-ring active-profile identity remains unmapped.
