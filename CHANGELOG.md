...
### Fixed
- DPI sliders and staged/clutch DPI writes now clamp to the selected device profile’s supported ceiling instead of using one global `30,000` DPI cap, so Basilisk V3 X HyperSpeed devices top out at `18,000` DPI while higher-end Basilisk models keep their broader ranges.
