...
- Added an automated Windows BTVS/tshark capture wrapper plus Basilisk V3 Pro Bluetooth profile-cycle capture notes. The new capture-backed research documents Synapse profile projection traffic on research-only keys such as `01 86 00 00`, `01 82 00 00`, `01 8C <target> 00`, and `08 05` / `08 06` / `08 07` profile-apply candidates.
...
- Added Synapse-closed Bluetooth physical profile-button captures. The draft spec now records that no in-window BLE vendor profile-cycle traffic identified the active profile, while a companion Windows HID sniff observed profile-cycle hint reports (`04 04 ...`, then `05 05 39 ...`) after each onboard button press. OpenSnek's target model is firmware-first: trust onboard profile cycling by default, use passive HID hints to trigger debounced one-shot live-profile fingerprint refreshes instead of continuous current-profile polling, and treat Synapse's software interception as a vendor UI behavior.
- Added a Windows event-driven Basilisk V3 Pro Bluetooth profile-cycle watcher. The follow-up read capture confirms passive HID hints can drive reactive UI refreshes without polling, but known live-target readback keys (`0B 84 01 00`, `08 84 01 04`) stayed unchanged after Synapse-closed hardware profile cycles, so those keys alone cannot identify the firmware-ring active profile.
- Mapped the Basilisk V3 Pro Bluetooth active hardware DPI surface for profile-cycle UI refreshes. `0B 81/82/83 00 00` expose the hardware-active DPI scalar/stage-list/stage-token, while stored slots `1..4` map to BLE targets `2..5`; OpenSnek can react to the passive profile-cycle HID hint and then read `0B 82 00 00` to infer the active onboard slot when stored DPI tables are unique.
- Added a Synapse startup takeover capture for Basilisk V3 Pro Bluetooth. The filtered startup trace shows Synapse applying/projecting the selected software profile through `08 05` / `08 07` / `08 06` and live DPI writes, while reading profile-button binding `08 84 <target> 6A`; no direct `08 04 <target> 6A` rewrite was observed, so OpenSnek should keep firmware-first profile-button behavior rather than copying Synapse's software ownership path.
- Tightened the Basilisk V3 Pro Bluetooth profile CRUD docs to distinguish the Synapse live projection target (`1`) from the hardware-active DPI identity surface (`0B 81/82/83 00 00`), and to mark `03 80` / `03 84` as read/inventory candidates rather than fully shipped profile inventory support.
- Filled out the Swift probe surface for Basilisk V3 Pro Bluetooth profile validation. `OpenSnekProbe` can now read stored-target DPI fingerprints and selected button slots, write a guarded stored-target button binding with optional live projection, decode duplicated or interleaved button readback lanes, and listen for passive Bluetooth HID profile-cycle hint reports.
- Added a live macOS no-stored-profile qualification for Basilisk V3 Pro Bluetooth profile-button monitoring. When `03 80 00 00` returned only target `1`, physical profile-button presses produced no passive HID hint and no active `0B 82 00 00` fingerprint change, consistent with the button being a firmware no-op when there is no cycleable onboard profile.
- Added live macOS Basilisk V3 Pro Bluetooth stored-profile validation. `OpenSnekProbe bt-profile-create` can now replay the capture-backed target-`2` create flow, register stored slot `1` in `03 80 00 00`, and confirm a physical profile-button press through the passive HID pair (`04 04 ...`, then `05 05 39 ...`) plus a follow-up `0B 82 00 00` active-fingerprint change.
- Live macOS profile probing now validates `03 82 00 00` as the Basilisk V3 Pro Bluetooth current active-target read. `OpenSnekProbe bt-profile-read` prints that direct target byte, with `0B 82 00 00` kept as validation/fallback instead of the primary profile identity path.
- Documented Basilisk V3 Pro Bluetooth profile metadata readback. `03 84 <target> 00` with offset/length payloads reads the device-backed UUID/name metadata written by `03 04 <target> 00`, while full profile hydration remains a multi-surface orchestration rather than a single mapped bulk profile read/write command.
- Documented Basilisk V3 Pro Bluetooth profile-scoped versus device-global surfaces. Stored brightness/static color now have target-scoped read/write paths, while advanced/effect lighting payloads remain unmapped; sleep timeout and battery/status remain device-global telemetry rather than profile data.
- Added stored-only Basilisk V3 Pro Bluetooth update validation and probe readback for inactive profile targets. Target-scoped DPI, Button5, brightness, and static scroll-wheel color writes now read back from stored target `3` without changing live projection target `1`; after firmware cycling to that profile, target `0` mirrors the active hardware DPI, button, and lighting state. `bt-profile-read` now prints target `0`, target `1`, and stored lighting/button surfaces side by side.
- Validated Basilisk V3 Pro Bluetooth target unassign behavior. `03 06 <target> 00` removes the target from `03 80` cycle inventory, but does not immediately erase its readable metadata/settings banks; if the target is currently active, firmware keeps reporting it through `03 82` until the next profile-cycle press skips to an inventory-listed target.
- Documented recreate-after-delete behavior and hardened `OpenSnekProbe bt-profile-create` to fail fast on non-success write ACKs, after a transient stored DPI stage write returned status `0x01` and left the target partially rewritten until the stage payload was retried.
- Validated Basilisk V3 Pro Bluetooth profile persistence across reconnect. Recreated target `3` kept its `03 80` inventory entry, `03 84` metadata, stored DPI, and stored brightness after a Bluetooth reconnect, while `03 82` reset to target `1` in that pass.
- Validated Basilisk V3 Pro Bluetooth active-target selector writes. `03 02 00 00` with payload `03` ACKed, moved `03 82 00 00` from target `1` to target `3`, and target `0` mirrored target `3`; payload `02` returned status `0x03` because target `2` was not in `03 80`, and payload `01` restored target `1`. `OpenSnekProbe bt-profile-active-set --target <id> --yes` now wraps the selector.
- Added a Basilisk V3 Pro USB profile CRUD draft from direct OpenSnek USB feature-report reads, without Synapse USB capture traffic. USB now has documented profile-addressed DPI scalar/stage, button, and brightness banks; storage IDs `2` and `3` match the profiles created over Bluetooth, while USB active-profile identity, metadata writes, and stored-bank write safety remained unresolved at that stage.
- Validated Basilisk V3 Pro USB physical profile-cycle detection. The profile button emits a passive HID hint pair (`04 00 ...` then `05 39 ...`) on the `0x01:0x06` keyboard-style interface, and effective storage/profile `0` mirrors the newly active onboard profile after the press; `00:87` stayed unchanged, so direct active-profile identity was not yet mapped at that stage.
- Cross-checked OpenRazer's unknown-command onboard-memory notes against the attached V3 Pro and mapped USB metadata reads through `05:88`. Slot `2` and `3` metadata chunks returned the same UUID/name data read over Bluetooth, including `OPENSNEK_MAC_SLOT_1` and `OPENSNEK_RECREATE_SLOT_2`; the likely `05:08` bulk write counterpart remains unmapped.
- Added a read-only `OpenSnekProbe usb-profile-read` sweep for USB profile research. It serially reads `00:87`, `05:88` metadata chunks, profile-addressed DPI scalar/stages, per-zone brightness, and selected button slots; the attached V3 Pro confirmed effective profile `0` matched stored profile `3`, while slots `4` and `5` also exposed readable metadata despite `00:87` reporting count `3`.
- Added `OpenSnekProbe usb-profile-verify-writes`, a guarded same-value USB stored-profile write verifier. The attached V3 Pro round-tripped profile `5` DPI scalar (`04:05`), DPI stages (`04:06`), and all three brightness zones (`0F:04`), with readback echo validation to reject stale USB feature-report replies.
- Added `OpenSnekProbe usb-profile-verify-changed-writes`, a guarded changed-value USB stored-profile verifier. It snapshots profile `5`, writes temporary DPI scalar, DPI stage, and brightness changes, validates readback, and restores the original values; the attached V3 Pro round-tripped all of those surfaces cleanly.
- Validated Basilisk V3 Pro USB stored-profile write persistence across reconnect. Temporary profile `5` DPI scalar, DPI stage, and brightness changes survived USB unplug/replug and were restored afterward; write ordering matters because a stage-table write can re-project the scalar surface, so guarded flows should write stages before the scalar.
- Fixed `OpenSnekProbe usb-raw --args` byte-list parsing so two-digit tokens such as `20` are interpreted as hex bytes, matching the documented probe examples and avoiding accidental decimal byte writes.
- Validated Basilisk V3 Pro USB stored-profile delete/unassign. `05:03 <profile>` removes a stored profile from the hardware cycle ring without erasing its readable settings/metadata bank; after `05:03 02`, the physical profile-cycle button skipped profile `2` and effective profile `0` moved to stored profile `3`. `OpenSnekProbe usb-profile-delete --profile <id> --yes` now wraps the command with exact echo validation.
- Validated Basilisk V3 Pro USB direct active-profile reads. `05:84`, size `00`, tracked profile-button transitions from profile `3` to base/profile `1` and back to profile `3`, while `00:87` stayed `02 32 03`; `OpenSnekProbe usb-profile-active-read` now wraps the direct read.
- Validated Basilisk V3 Pro USB active-profile selector writes. `05:04 01` and `05:04 03` ACKed, updated `05:84`, and projected effective profile `0`; `05:04 02`, `05:04 04`, and `05:04 05` returned status `0x03` in the current device state, so `OpenSnekProbe usb-profile-active-set --profile <id> --yes` now wraps the assigned-profile selector.
- Fixed Basilisk V3 Pro USB active-profile hydration to parse the validated `05:84` response shape even when the response size byte is `0x00`, and to prefer that direct active-profile register over the stale `00:87` active hint. A follow-up read-only pass showed `05:80`, `05:81`, and `05:8A` are still summary/list candidates only, not a trusted side-effect-free inventory.
- Disabled the USB metadata-write probe. A fuller OpenRazer-style `06:8E` / `05:02` / multi-chunk `05:08` attempt proved `05:08` is unsafe as a metadata-only command: profile `5` metadata was erased to UUID `ffffffff-ffff-ffff-ffff-ffffffffffff`, name `nil`, and settings were disturbed. Settings were restored through validated DPI/lighting writes, but metadata remains unrecovered and USB metadata writes remain unmapped.
- Documented the Bluetooth-to-USB create/name mapping boundary. Bluetooth has a guarded explicit-target create/rewrite path with device-backed UUID/name metadata, but USB does not yet have a safe create or rename equivalent: read-only probes showed `06:8E` is profile-independent capability data, `05:82` rejects profile arguments, and `05:8A` ignores profile arguments, leaving `05:08` blocked until the complete bulk blob and transaction are mapped.
