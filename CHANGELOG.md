...
### Added
- USB devices that advertise multiple onboard profiles now get an `Onboard Profiles` card in the macOS UI. It shows every reported slot, reads their button-remap state in the background, and marks which slots are still factory-default versus customized.
...
- Reconnected devices now trigger an immediate selected-device state refresh instead of waiting for the slower reconnect polling cadence, and the service runtime temporarily uses the faster interactive presence/state cadence while a selected device is visible but still reconnecting or missing live state.
- USB multi-profile mice no longer eagerly read every stored button-profile slot during normal connect hydration. Stored-slot summaries now hydrate only when the profile UI asks for them, and stale USB button readback no longer overwrites a newer local edit that landed while the read was still in flight.
- Connect-time persisted lighting restore is now limited to the Basilisk V3 X HyperSpeed path. The Basilisk V3 Pro and Basilisk V3 35K are treated as remembering their own lighting state, which avoids unnecessary reconnect-time lighting writes on those devices.
- USB reconnects that come back under a new location-based device ID now keep showing the last known state while telemetry recovers, and reconnect-time `telemetry unavailable` reads no longer clear that seeded state as if the mouse had fully disappeared.
...
- The macOS `Buttons` card now shows a persistent `Loaded from ...` label for the current source, disables reloading the always-live slot `1` from the `Load` popover, and keeps profile management focused on explicit `Load`, `Store`, and `Manage` actions without extra reset affordances.
- Probing on the attached Basilisk V3 35K confirmed that slot `0x0F` is editable after all: the native clutch slot accepts remap writes, preserves its distinct default block `06 01 05 01 90 01 90`, and the shared `DPI Clutch` action now works on the 35K just as it does on the V3 Pro.
