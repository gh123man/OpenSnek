...
- Fixed Basilisk V3 Pro profile switches and reconnect hydration so reduced DPI stage tables keep driving live DPI-cycle UI updates and known advanced onboard lighting effects do not collapse to "Onboard Static" after profile hydration.
- Fixed Basilisk V3 Pro USB Advanced lighting reconnect recovery so sleeping mouse wakeups reassert the selected animation, detect stale/missing service lighting status, and replay persisted Apply on connect effects instead of staying on onboard static.
