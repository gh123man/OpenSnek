...
- Background-service clients now receive lightweight per-device live state updates for rapid DPI changes instead of depending on full cross-process snapshots for every passive HID or fast-poll refresh, which reduces menu-bar-mode lag and stale last-poll UI updates on Bluetooth.
- Remote app/service realtime state delivery now runs over a persistent localhost subscription on the existing background-service socket instead of `DistributedNotificationCenter`, which removes the old cross-process notification bottleneck from the DPI hot path for both USB and Bluetooth devices.
